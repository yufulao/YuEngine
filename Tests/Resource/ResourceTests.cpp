...
using yuengine::resource::ResourceDescriptorBatchTypeEnumerationResult;
...
constexpr const char *TEST_DESCRIPTOR_BATCH_TYPE_ENUMERATION =
    "Resource_DescriptorBatchTypeEnumerationReturnsAtomicRows";
...
constexpr const char *TEST_DEPENDENCY_EDGE_COUNT_ENUMERATION =
    "Resource_DependencyEdgeCountSnapshotMatchesEnumeration";
...
int ResourceDescriptorTypeEnumerationReturnsMatchingDescriptors() {
    ResourceRegistry registry;
    const ResourceRegistrationResult first_texture_result = registry.RegisterSyntheticDescriptor(
        DescriptorWithReferenceCount(TYPE_TEXTURE, "type_enum_texture_a", 2U));
    if (!first_texture_result.Succeeded()) {
        return Fail("descriptor type enumeration first texture registration failed");
    }

    const ResourceRegistrationResult material_result = registry.RegisterSyntheticDescriptor(
        DescriptorWithReferenceCount(TYPE_MATERIAL, "type_enum_material", 1U));
    if (!material_result.Succeeded()) {
        return Fail("descriptor type enumeration material registration failed");
    }

    const ResourceRegistrationResult second_texture_result = registry.RegisterSyntheticDescriptor(
        DescriptorWithReferenceCount(TYPE_TEXTURE, "type_enum_texture_b", 4U));
    if (!second_texture_result.Succeeded()) {
        return Fail("descriptor type enumeration second texture registration failed");
    }

    const ResourceRegistrationResult audio_result = registry.RegisterSyntheticDescriptor(
        DescriptorWithReferenceCount(TYPE_AUDIO, "type_enum_audio", 3U));
    if (!audio_result.Succeeded()) {
        return Fail("descriptor type enumeration audio registration failed");
    }

    const ResourceRegistrationResult third_texture_result = registry.RegisterSyntheticDescriptor(
        DescriptorWithReferenceCount(TYPE_TEXTURE, "type_enum_texture_c", 6U));
    if (!third_texture_result.Succeeded()) {
        return Fail("descriptor type enumeration third texture registration failed");
    }

    std::array<ResourceDescriptorLookupRecord, 3U> output_records{};
    std::uint32_t output_record_count = 99U;
    const ResourceSnapshot before_success_snapshot = registry.Snapshot();
    const ResourceDescriptorTypeEnumerationResult success_result =
    ...
    if (!success_result.Succeeded() ||
        success_result.matched_descriptor_count != 3U ||
        output_record_count != 3U) {
        return Fail("descriptor type enumeration success returned wrong counts");
    }

    const bool first_record_matches = LookupRecordMatches(
        output_records[0U],
        first_texture_result.handle,
        TYPE_TEXTURE,
        "type_enum_texture_a",
        2U);
    const bool second_record_matches = LookupRecordMatches(
        output_records[1U],
        second_texture_result.handle,
        TYPE_TEXTURE,
        "type_enum_texture_b",
        4U);
    const bool third_record_matches = LookupRecordMatches(
        output_records[2U],
        third_texture_result.handle,
        TYPE_TEXTURE,
        "type_enum_texture_c",
        6U);
    if (!first_record_matches || !second_record_matches || !third_record_matches) {
        return Fail("descriptor type enumeration did not preserve type registration order");
    }
    ...
    if (after_success_snapshot.registered_resource_count != before_success_snapshot.registered_resource_count ||
        after_success_snapshot.type_count != before_success_snapshot.type_count ||
        after_success_snapshot.dependency_validation_count != before_success_snapshot.dependency_validation_count) {
        return Fail("descriptor type enumeration success mutated counters");
    }

    const std::array<ResourceDescriptorLookupRecord, 3U> stable_records = output_records;
    const std::uint32_t stable_output_record_count = output_record_count;
    const ResourceDescriptorTypeEnumerationResult capacity_result =
        registry.EnumerateSyntheticDescriptorsByType(
            TYPE_TEXTURE,
            output_records.data(),
            2U,
    ...
    if (capacity_result.status != ResourceStatus::CapacityExceeded ||
        capacity_result.required_descriptor_count != 3U ||
        output_record_count != stable_output_record_count) {
        return Fail("descriptor type enumeration capacity failure missed atomic count or required count");
    }

    const bool capacity_first_preserved = LookupRecordEquals(output_records[0U], stable_records[0U]);
    const bool capacity_second_preserved = LookupRecordEquals(output_records[1U], stable_records[1U]);
    const bool capacity_third_preserved = LookupRecordEquals(output_records[2U], stable_records[2U]);
    if (!capacity_first_preserved || !capacity_second_preserved || !capacity_third_preserved) {
        return Fail("descriptor type enumeration capacity failure mutated output rows");
    }

    const ResourceSnapshot after_capacity_snapshot = registry.Snapshot();
    if (after_capacity_snapshot.registered_resource_count != after_success_snapshot.registered_resource_count ||
        after_capacity_snapshot.type_count != after_success_snapshot.type_count ||
        after_capacity_snapshot.dependency_validation_count != after_success_snapshot.dependency_validation_count ||
        after_capacity_snapshot.last_required_resource_count != 3U) {
        return Fail("descriptor type enumeration capacity failure mutated counters or missed snapshot count");
    }

    const ResourceTypeId invalid_type{};
    const ResourceDescriptorTypeEnumerationResult invalid_type_result =
        registry.EnumerateSyntheticDescriptorsByType(
            invalid_type,
            output_records.data(),
            static_cast<std::uint32_t>(output_records.size()),
    ...
    if (invalid_type_result.status != ResourceStatus::InvalidDescriptor ||
        output_record_count != stable_output_record_count) {
        return Fail("descriptor type enumeration invalid type mutated count");
    }

    const bool invalid_first_preserved = LookupRecordEquals(output_records[0U], stable_records[0U]);
    const bool invalid_second_preserved = LookupRecordEquals(output_records[1U], stable_records[1U]);
    const bool invalid_third_preserved = LookupRecordEquals(output_records[2U], stable_records[2U]);
    if (!invalid_first_preserved || !invalid_second_preserved || !invalid_third_preserved) {
        return Fail("descriptor type enumeration invalid type mutated output rows");
    }

    const ResourceDescriptorTypeEnumerationResult null_output_result =
        registry.EnumerateSyntheticDescriptorsByType(
            TYPE_TEXTURE,
            nullptr,
            static_cast<std::uint32_t>(output_records.size()),
    ...
    if (null_output_result.status != ResourceStatus::InvalidHandle ||
        output_record_count != stable_output_record_count) {
        return Fail("descriptor type enumeration null output missed invalid handle or mutated count");
    }

    std::uint32_t missing_type_count = 77U;
    const ResourceDescriptorTypeEnumerationResult missing_type_result =
        registry.EnumerateSyntheticDescriptorsByType(
            TYPE_EFFECT,
            nullptr,
            0U,
...
int ResourceDescriptorBatchTypeEnumerationReturnsAtomicRows() {
    ResourceRegistry registry;
    const ResourceRegistrationResult first_texture_result = registry.RegisterSyntheticDescriptor(
        DescriptorWithReferenceCount(TYPE_TEXTURE, "batch_type_enum_texture_a", 2U));
    if (!first_texture_result.Succeeded()) {
        return Fail("descriptor batch type enumeration first texture registration failed");
    }

    const ResourceRegistrationResult material_result = registry.RegisterSyntheticDescriptor(
        DescriptorWithReferenceCount(TYPE_MATERIAL, "batch_type_enum_material", 1U));
    if (!material_result.Succeeded()) {
        return Fail("descriptor batch type enumeration material registration failed");
    }

    const ResourceRegistrationResult audio_result = registry.RegisterSyntheticDescriptor(
        DescriptorWithReferenceCount(TYPE_AUDIO, "batch_type_enum_audio", 3U));
    if (!audio_result.Succeeded()) {
        return Fail("descriptor batch type enumeration audio registration failed");
    }

    const ResourceRegistrationResult second_texture_result = registry.RegisterSyntheticDescriptor(
        DescriptorWithReferenceCount(TYPE_TEXTURE, "batch_type_enum_texture_b", 4U));
    if (!second_texture_result.Succeeded()) {
        return Fail("descriptor batch type enumeration second texture registration failed");
    }
    ...
    const std::array<ResourceTypeId, 4U> success_types{{
        ...
        TYPE_EFFECT,
        TYPE_TEXTURE}};
    std::array<ResourceDescriptorLookupRecord, 5U> output_records{};
    std::uint32_t output_record_count = 99U;
    const ResourceSnapshot before_success_snapshot = registry.Snapshot();
    const ResourceDescriptorBatchTypeEnumerationResult success_result =
        registry.EnumerateSyntheticDescriptorsByType(
            success_types.data(),
            static_cast<std::uint32_t>(success_types.size()),
            output_records.data(),
            ...
            &output_record_count);
    if (!success_result.Succeeded() ||
        success_result.matched_descriptor_count != 5U ||
        output_record_count != 5U) {
        return Fail("descriptor batch type enumeration success returned wrong output count");
    }
    ...
    const bool first_record_matches = LookupRecordMatches(
        ...
        first_texture_result.handle,
        TYPE_TEXTURE,
        "batch_type_enum_texture_a",
        2U);
    const bool second_record_matches = LookupRecordMatches(
        output_records[1U],
        second_texture_result.handle,
        TYPE_TEXTURE,
        "batch_type_enum_texture_b",
        4U);
    const bool third_record_matches = LookupRecordMatches(
        output_records[2U],
        material_result.handle,
        TYPE_MATERIAL,
        "batch_type_enum_material",
        1U);
    const bool fourth_record_matches = LookupRecordMatches(
        output_records[3U],
        first_texture_result.handle,
        TYPE_TEXTURE,
        "batch_type_enum_texture_a",
        2U);
    const bool fifth_record_matches = LookupRecordMatches(
        output_records[4U],
        second_texture_result.handle,
        TYPE_TEXTURE,
        "batch_type_enum_texture_b",
        4U);
    if (!first_record_matches ||
        !second_record_matches ||
        !third_record_matches ||
        !fourth_record_matches ||
        !fifth_record_matches) {
        return Fail("descriptor batch type enumeration did not preserve input and registry order");
    }
    ...
    if (after_success_snapshot.registered_resource_count != before_success_snapshot.registered_resource_count ||
        after_success_snapshot.type_count != before_success_snapshot.type_count ||
        after_success_snapshot.dependency_validation_count != before_success_snapshot.dependency_validation_count) {
        return Fail("descriptor batch type enumeration success mutated counters");
    }

    const std::array<ResourceDescriptorLookupRecord, 5U> stable_records = output_records;
    const std::uint32_t stable_output_record_count = output_record_count;
    const ResourceDescriptorBatchTypeEnumerationResult capacity_result =
        registry.EnumerateSyntheticDescriptorsByType(
            success_types.data(),
            static_cast<std::uint32_t>(success_types.size()),
            output_records.data(),
            ...
            &output_record_count);
    if (capacity_result.status != ResourceStatus::CapacityExceeded ||
        capacity_result.required_descriptor_count != 5U ||
        output_record_count != stable_output_record_count) {
        return Fail("descriptor batch type enumeration capacity failure missed required count or mutated count");
    }

    const bool capacity_first_preserved = LookupRecordEquals(output_records[0U], stable_records[0U]);
    const bool capacity_second_preserved = LookupRecordEquals(output_records[1U], stable_records[1U]);
    const bool capacity_third_preserved = LookupRecordEquals(output_records[2U], stable_records[2U]);
    const bool capacity_fourth_preserved = LookupRecordEquals(output_records[3U], stable_records[3U]);
    const bool capacity_fifth_preserved = LookupRecordEquals(output_records[4U], stable_records[4U]);
    if (!capacity_first_preserved ||
        !capacity_second_preserved ||
        !capacity_third_preserved ||
        !capacity_fourth_preserved ||
        !capacity_fifth_preserved) {
        return Fail("descriptor batch type enumeration capacity failure mutated output rows");
    }
    ...
    if (after_capacity_snapshot.registered_resource_count != after_success_snapshot.registered_resource_count ||
        after_capacity_snapshot.type_count != after_success_snapshot.type_count ||
        after_capacity_snapshot.dependency_validation_count != after_success_snapshot.dependency_validation_count ||
        after_capacity_snapshot.last_required_resource_count != 5U) {
        return Fail("descriptor batch type enumeration capacity failure mutated counters or missed snapshot count");
    }
    ...
    const std::array<ResourceTypeId, 2U> invalid_types{{
        TYPE_TEXTURE,
        invalid_type}};
    const ResourceDescriptorBatchTypeEnumerationResult invalid_type_result =
        registry.EnumerateSyntheticDescriptorsByType(
            invalid_types.data(),
            static_cast<std::uint32_t>(invalid_types.size()),
            output_records.data(),
    ...
    if (invalid_type_result.status != ResourceStatus::InvalidDescriptor ||
        invalid_type_result.failed_type_index != 1U ||
        output_record_count != stable_output_record_count) {
        return Fail("descriptor batch type enumeration invalid type missed failed index or mutated count");
    }

    const bool invalid_first_preserved = LookupRecordEquals(output_records[0U], stable_records[0U]);
    const bool invalid_second_preserved = LookupRecordEquals(output_records[1U], stable_records[1U]);
    const bool invalid_third_preserved = LookupRecordEquals(output_records[2U], stable_records[2U]);
    const bool invalid_fourth_preserved = LookupRecordEquals(output_records[3U], stable_records[3U]);
    const bool invalid_fifth_preserved = LookupRecordEquals(output_records[4U], stable_records[4U]);
    if (!invalid_first_preserved ||
        !invalid_second_preserved ||
        !invalid_third_preserved ||
        !invalid_fourth_preserved ||
        !invalid_fifth_preserved) {
        return Fail("descriptor batch type enumeration invalid type mutated output rows");
    }

    const ResourceDescriptorBatchTypeEnumerationResult null_types_result =
        registry.EnumerateSyntheticDescriptorsByType(
            nullptr,
            static_cast<std::uint32_t>(success_types.size()),
            output_records.data(),
            static_cast<std::uint32_t>(output_records.size()),
            &output_record_count);
    if (null_types_result.status != ResourceStatus::InvalidDescriptor ||
        output_record_count != stable_output_record_count) {
        return Fail("descriptor batch type enumeration null types mutated count");
    }

    const ResourceDescriptorBatchTypeEnumerationResult null_output_result =
        registry.EnumerateSyntheticDescriptorsByType(
            success_types.data(),
            static_cast<std::uint32_t>(success_types.size()),
            nullptr,
    ...
    if (null_output_result.status != ResourceStatus::InvalidHandle ||
        output_record_count != stable_output_record_count) {
        return Fail("descriptor batch type enumeration null output mutated count");
    }

    const std::array<ResourceTypeId, 1U> missing_types{{TYPE_EFFECT}};
    std::uint32_t missing_output_count = 55U;
    const ResourceDescriptorBatchTypeEnumerationResult missing_type_result =
        registry.EnumerateSyntheticDescriptorsByType(
            missing_types.data(),
            static_cast<std::uint32_t>(missing_types.size()),
            nullptr,
            0U,
            &missing_output_count);
    if (!missing_type_result.Succeeded() ||
        missing_type_result.matched_descriptor_count != 0U ||
        missing_output_count != 0U) {
        return Fail("descriptor batch type enumeration missing type did not return empty success");
    }

    std::uint32_t empty_output_count = 44U;
    const ResourceDescriptorBatchTypeEnumerationResult empty_result =
        registry.EnumerateSyntheticDescriptorsByType(
            nullptr,
            0U,
            nullptr,
            ...
            &empty_output_count);
    if (!empty_result.Succeeded() || empty_output_count != 0U) {
        return Fail("descriptor batch type enumeration empty batch did not succeed");
    }
...
int ResourceDependencyEdgeCountSnapshotMatchesEnumeration() {
    ResourceRegistry empty_registry;
    std::uint32_t empty_dependency_count = 7U;
    const ResourceSnapshot empty_before_snapshot = empty_registry.Snapshot();
    const ResourceStatus empty_status = empty_registry.CountDependencyEdges(&empty_dependency_count);
    if (empty_status != ResourceStatus::Success || empty_dependency_count != 0U) {
        return Fail("dependency edge count empty graph did not return zero");
    }
    ...
    const ResourceDependencyBatchResult batch_result = registry.AddDependencies(
        ...
        static_cast<std::uint32_t>(batch_rows.size()));
    if (!batch_result.Succeeded()) {
        return Fail("dependency edge count batch fixture failed");
    }

    std::array<ResourceDependencyRequest, 3U> output_dependencies{};
    std::uint32_t enumerated_dependency_count = 0U;
    const ResourceStatus enumerate_status = registry.EnumerateDependencyEdges(
        output_dependencies.data(),
        static_cast<std::uint32_t>(output_dependencies.size()),
        &enumerated_dependency_count);
    if (enumerate_status != ResourceStatus::Success || enumerated_dependency_count != 3U) {
        return Fail("dependency edge count fixture enumeration failed");
    }

    std::uint32_t count_snapshot = 77U;
    const ResourceSnapshot before_count_snapshot = registry.Snapshot();
    const ResourceStatus count_status = registry.CountDependencyEdges(&count_snapshot);
    if (count_status != ResourceStatus::Success || count_snapshot != enumerated_dependency_count) {
        return Fail("dependency edge count did not match enumeration count");
    }
    ...
    const ResourceStatus capacity_status = registry.EnumerateDependencyEdges(
        ...
        &required_dependency_count);
    if (capacity_status != ResourceStatus::CapacityExceeded ||
        required_dependency_count != stable_count_snapshot) {
        return Fail("dependency edge count setup capacity enumeration missed required count");
    }

    const ResourceSnapshot after_capacity_snapshot = registry.Snapshot();
    if (after_capacity_snapshot.dependency_edge_count != before_capacity_snapshot.dependency_edge_count) {
        return Fail("dependency edge count setup capacity enumeration mutated edge count");
    }

    std::uint32_t after_capacity_count = 0U;
    const ResourceStatus after_capacity_count_status = registry.CountDependencyEdges(&after_capacity_count);
    if (after_capacity_count_status != ResourceStatus::Success ||
        after_capacity_count != stable_count_snapshot) {
        return Fail("dependency edge count changed after capacity enumeration failure");
    }

    std::array<ResourceHandle, 2U> traversal_dependencies{};
    std::uint32_t traversal_dependency_count = 0U;
    const ResourceStatus traversal_status = registry.TraverseDependencies(
        root.handle,
        traversal_dependencies.data(),
        static_cast<std::uint32_t>(traversal_dependencies.size()),
        &traversal_dependency_count);
    if (traversal_status != ResourceStatus::Success || traversal_dependency_count != 2U) {
        return Fail("dependency edge count traversal setup failed");
    }

    std::uint32_t after_traversal_count = 0U;
    const ResourceStatus after_traversal_status = registry.CountDependencyEdges(&after_traversal_count);
    if (after_traversal_status != ResourceStatus::Success ||
        after_traversal_count != stable_count_snapshot) {
        return Fail("dependency edge count changed after traversal");
    }

    std::array<ResourceDependencyRequest, 3U> after_output_dependencies{};
    std::uint32_t after_output_dependency_count = 0U;
    const ResourceStatus after_enumerate_status = registry.EnumerateDependencyEdges(
        after_output_dependencies.data(),
        static_cast<std::uint32_t>(after_output_dependencies.size()),
        &after_output_dependency_count);
    if (after_enumerate_status != ResourceStatus::Success ||
        after_output_dependency_count != stable_count_snapshot) {
        return Fail("dependency edge count final enumeration failed");
    }

    if (!DependencyRequestMatches(after_output_dependencies[0U], root.handle, shared.handle) ||
        !DependencyRequestMatches(after_output_dependencies[1U], shared.handle, leaf.handle) ||
        !DependencyRequestMatches(after_output_dependencies[2U], second_root.handle, shared.handle)) {
        return Fail("dependency edge count operations changed direct edge order");
    }
...
int main(int argc, char** argv) {
    ...
    const TestRegistry test_registry{
        ...
        {TEST_DESCRIPTOR_BATCH_TYPE_ENUMERATION, ResourceDescriptorBatchTypeEnumerationReturnsAtomicRows},
        ...
        {TEST_DEPENDENCY_EDGE_COUNT_ENUMERATION, ResourceDependencyEdgeCountSnapshotMatchesEnumeration},
