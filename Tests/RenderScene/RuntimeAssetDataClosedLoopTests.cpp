...
using yuengine::runtimeasset::CommitRuntimeAssetDataAssetDependencyBatch;
...
using yuengine::runtimeasset::EnumerateRuntimeAssetDataAssetDependenciesByTypes;
...
using yuengine::runtimeasset::RuntimeAssetDataAssetDependencyBatchTypeEnumerationRequest;
using yuengine::runtimeasset::RuntimeAssetDataAssetDependencyBatchTypeEnumerationResult;
...
constexpr const char *TEST_GENERIC_RENDER_SCENE_SUBMISSION_MATERIAL_TABLE_CAPACITY =
    "RuntimeAssetData_GenericRenderSceneSubmissionRejectsSmallMaterialTableWithoutMutation";
constexpr const char *TEST_GENERIC_RENDER_SCENE_SUBMISSION_DUPLICATE_MATERIAL_ID =
...
constexpr const char *TEST_ANIMATION_TARGET_BINDING =
    "RuntimeAssetData_AnimationTrackTargetBindingResolvesTargetIdAndProperty";
constexpr const char *TEST_ANIMATION_TARGET_BINDING_MISSING_TARGET =
    "RuntimeAssetData_AnimationTrackTargetBindingRejectsMissingTargetWithoutMutation";
constexpr const char *TEST_ANIMATION_TARGET_BINDING_UNSUPPORTED_PROPERTY =
...
constexpr const char *TEST_RUNTIME_INSTANCE_MAPPING =
...
constexpr const char *TEST_RUNTIME_INSTANCE_MAPPING_CAPACITY =
    "RuntimeAssetData_RuntimeInstanceMappingRejectsCapacityOverflowWithoutMutation";
constexpr const char *TEST_RUNTIME_INSTANCE_MAPPING_TARGET_FAMILY =
...
constexpr const char *TEST_RUNTIME_ASSET_DEPENDENCY_BATCH =
...
constexpr const char *TEST_RUNTIME_DEPENDENCY_BATCH_TYPE_ENUMERATION =
    "RuntimeAssetData_AssetDependencyBatchTypeEnumerationIsAtomic";
...
struct LoadedGraph final {
    ...
    std::array<RuntimeAssetAnimationTrackTargetBindingRecord, 1U> animation_target_bindings{};
...
struct BoundedLoadedGraph final {
    ...
    std::array<RuntimeAssetTargetIdentityRecord, 3U> target_identities{};
    std::array<RuntimeAssetRuntimeInstanceMappingRecord, 3U> runtime_instance_mappings{};
    std::array<AnimationRuntimeClipRecord, 2U> animation_clips{};
    std::array<AnimationRuntimeTrackRecord, 3U> animation_tracks{};
    std::array<RuntimeAssetAnimationTrackTargetBindingRecord, 3U> animation_target_bindings{};
...
std::string BoundedTargetBindingAnimationBytes() {
    return std::string(
        "YUASSET ANIMATION 1\n"
        "schema=rav0-source\n"
        "id=bounded_target_binding\n"
        "target=scene_entity:101\n"
        "track=transform:rotation_y\n"
        "clips=1\n"
        "tracks=2\n"
        "keyframes=4\n"
        "clip0=id=1|duration=1|first_track_index=0|track_count=2|sample_rate=30\n"
        "track0=id=1|target_id=1001|property=transform:rotation_y|first_key=0|key_count=2|interp=linear\n"
...
void SeedGenericRenderSceneSubmissionSentinels(
    ...
    for (std::size_t index = 0U; index < frame_entities.size(); ++index) {
        ...
        entity.material_table_index = 9500U + static_cast<std::uint32_t>(index);
...
void SeedGenericRenderSceneSubmissionMaterialSentinels(
    std::span<RenderSceneRuntimeMaterialRecord> materials) {
    for (std::size_t index = 0U; index < materials.size(); ++index) {
        RenderSceneRuntimeMaterialRecord &material = materials[index];
        material.material_id = 9600U + static_cast<std::uint32_t>(index);
        material.material_constant_byte_count = index;
    }
}

bool GenericRenderSceneSubmissionMaterialSentinelsUnchanged(
    std::span<const RenderSceneRuntimeMaterialRecord> materials) {
    for (std::size_t index = 0U; index < materials.size(); ++index) {
        const RenderSceneRuntimeMaterialRecord &material = materials[index];
        if (material.material_id != 9600U + static_cast<std::uint32_t>(index) ||
            material.material_constant_byte_count != index) {
            return FailStep("generic render scene submission failure mutated material output");
        }
    }
...
RenderSceneRuntimeMaterialRecord MakeGenericRenderSceneSubmissionVariantMaterial(
    const RenderSceneRuntimeMaterialRecord &material,
    std::uint32_t material_id) {
    RenderSceneRuntimeMaterialRecord variant = material;
    variant.material_id = material_id;
    if (variant.material_constant_byte_count > 0U) {
        ++variant.material_constant_bytes[0U];
    }
...
bool LoadRuntimeAssetRecords(
    ...
    load_request.animation_target_bindings = graph.animation_target_bindings.data();
    load_request.animation_target_binding_capacity =
        static_cast<std::uint32_t>(graph.animation_target_bindings.size());
...
bool LoadBoundedRuntimeAssetRecords(
    ...
    load_request.target_identities = graph.target_identities.data();
    load_request.target_identity_capacity = static_cast<std::uint32_t>(graph.target_identities.size());
    load_request.runtime_instance_mappings = graph.runtime_instance_mappings.data();
    load_request.runtime_instance_mapping_capacity =
        static_cast<std::uint32_t>(graph.runtime_instance_mappings.size());
    load_request.animation_clips = graph.animation_clips.data();
    load_request.animation_clip_capacity = static_cast<std::uint32_t>(graph.animation_clips.size());
    load_request.animation_tracks = graph.animation_tracks.data();
    load_request.animation_track_capacity = static_cast<std::uint32_t>(graph.animation_tracks.size());
...
bool BoundedRuntimeInstanceMappingOutputTableMatches(const BoundedLoadedGraph &graph) {
    if (graph.scene_output.runtime_instance_mapping_count != 3U ||
        graph.scene_output.runtime_instance_mapping_capacity !=
            static_cast<std::uint32_t>(graph.runtime_instance_mappings.size())) {
        return FailStep("bounded runtime instance mapping counts changed");
    }

    const RuntimeAssetRuntimeInstanceMappingRecord &scene_node = graph.runtime_instance_mappings[0U];
    if (scene_node.target_kind != RuntimeAssetTargetIdentityKind::SceneNode ||
        scene_node.target_id != 1001U ||
        scene_node.scene_entity_id != 1U ||
        scene_node.scene_entity_index != 2U ||
        scene_node.scene_transform_index != 2U ||
        !scene_node.is_valid) {
        return FailStep("bounded scene node runtime instance mapping changed");
    }

    const RuntimeAssetRuntimeInstanceMappingRecord &model_node = graph.runtime_instance_mappings[1U];
    if (model_node.target_kind != RuntimeAssetTargetIdentityKind::ModelNode ||
        model_node.target_id != 2001U ||
        model_node.scene_entity_id != 1U ||
        model_node.scene_entity_index != 2U ||
        model_node.scene_transform_index != 2U ||
        !model_node.is_valid) {
        return FailStep("bounded model node runtime instance mapping changed");
    }

    const RuntimeAssetRuntimeInstanceMappingRecord &skeleton_joint = graph.runtime_instance_mappings[2U];
    if (skeleton_joint.target_kind != RuntimeAssetTargetIdentityKind::SkeletonJoint ||
        skeleton_joint.target_id != 3001U ||
        skeleton_joint.scene_entity_id != 1U ||
        skeleton_joint.scene_entity_index != 2U ||
        skeleton_joint.scene_transform_index != 2U ||
        !skeleton_joint.is_valid) {
        return FailStep("bounded skeleton joint runtime instance mapping changed");
    }

    return true;
}

bool BoundedAnimationTargetBindingOutputTableMatchesTargetIds(
    const BoundedLoadedGraph &graph,
    std::uint64_t rotation_target_id,
    std::uint64_t translation_target_id) {
    if (graph.scene_output.animation_target_binding_count != 2U ||
        graph.scene_output.animation_target_binding_capacity !=
            static_cast<std::uint32_t>(graph.animation_target_bindings.size())) {
        return FailStep("bounded animation target binding output counts changed");
    }

    const RuntimeAssetAnimationTrackTargetBindingRecord &rotation_binding = graph.animation_target_bindings[0U];
    if (rotation_binding.track_id != 1U ||
        rotation_binding.target_id != rotation_target_id ||
        rotation_binding.property != RuntimeAssetAnimationTargetProperty::TransformRotationY ||
        !rotation_binding.is_valid) {
        return FailStep("bounded rotation target binding changed");
    }

    const RuntimeAssetAnimationTrackTargetBindingRecord &translation_binding = graph.animation_target_bindings[1U];
    if (translation_binding.track_id != 2U ||
        translation_binding.target_id != translation_target_id ||
        translation_binding.property != RuntimeAssetAnimationTargetProperty::TransformTranslationY ||
        !translation_binding.is_valid) {
        return FailStep("bounded translation target binding changed");
    }

    const AnimationRuntimeTrackRecord &rotation_track = graph.animation_tracks[0U];
    if (rotation_track.target.value != 101U ||
        rotation_track.channel != AnimationRuntimeChannel::RotationY ||
        !rotation_track.is_valid) {
        return FailStep("bounded target binding did not resolve rotation track");
    }

    const AnimationRuntimeTrackRecord &translation_track = graph.animation_tracks[1U];
    if (translation_track.target.value != 101U ||
        translation_track.channel != AnimationRuntimeChannel::TranslationY ||
        !translation_track.is_valid) {
        return FailStep("bounded target binding did not resolve translation track");
    }

    return true;
}

bool BoundedAnimationTargetBindingOutputTableMatches(const BoundedLoadedGraph &graph) {
    return BoundedAnimationTargetBindingOutputTableMatchesTargetIds(graph, 1001U, 1001U);
}
...
void SeedBoundedSceneLoaderFailureSentinels(
    ...
    if (output != nullptr) {
        ...
        output->runtime_instance_mapping_count = 1784U;
    }
}
...
void SeedAnimationTargetBindingFailureSentinels(
    std::span<RuntimeAssetAnimationTrackTargetBindingRecord> bindings) {
    for (std::size_t index = 0U; index < bindings.size(); ++index) {
        bindings[index].track_id = 9101U + static_cast<std::uint32_t>(index);
        bindings[index].target_id = 9201U + static_cast<std::uint64_t>(index);
        bindings[index].property = RuntimeAssetAnimationTargetProperty::TransformScaleZ;
        bindings[index].is_valid = true;
    }
}

bool AnimationTargetBindingFailureSentinelsUnchanged(
    std::span<const RuntimeAssetAnimationTrackTargetBindingRecord> bindings) {
    for (std::size_t index = 0U; index < bindings.size(); ++index) {
        if (bindings[index].track_id != 9101U + static_cast<std::uint32_t>(index) ||
            bindings[index].target_id != 9201U + static_cast<std::uint64_t>(index) ||
            bindings[index].property != RuntimeAssetAnimationTargetProperty::TransformScaleZ ||
            !bindings[index].is_valid) {
            return FailStep("animation target binding failure mutated output");
        }
    }
...
void SeedRuntimeInstanceMappingFailureSentinels(
    std::span<RuntimeAssetRuntimeInstanceMappingRecord> mappings) {
    for (std::size_t index = 0U; index < mappings.size(); ++index) {
        mappings[index].target_kind = RuntimeAssetTargetIdentityKind::SkeletonJoint;
        mappings[index].target_id = 9301U + static_cast<std::uint64_t>(index);
        mappings[index].scene_entity_id = 9401U + static_cast<std::uint32_t>(index);
        mappings[index].scene_entity_index = 9501U + static_cast<std::uint32_t>(index);
        mappings[index].scene_transform_index = 9601U + static_cast<std::uint32_t>(index);
        mappings[index].is_valid = true;
    }
}

bool RuntimeInstanceMappingFailureSentinelsUnchanged(
    std::span<const RuntimeAssetRuntimeInstanceMappingRecord> mappings) {
    for (std::size_t index = 0U; index < mappings.size(); ++index) {
        if (mappings[index].target_kind != RuntimeAssetTargetIdentityKind::SkeletonJoint ||
            mappings[index].target_id != 9301U + static_cast<std::uint64_t>(index) ||
            mappings[index].scene_entity_id != 9401U + static_cast<std::uint32_t>(index) ||
            mappings[index].scene_entity_index != 9501U + static_cast<std::uint32_t>(index) ||
            mappings[index].scene_transform_index != 9601U + static_cast<std::uint32_t>(index) ||
            !mappings[index].is_valid) {
            return FailStep("runtime instance mapping failure mutated output");
        }
    }
...
bool ProbeBoundedSceneLoaderFailureWithoutOutputMutation(
    ...
    SeedAnimationTargetBindingFailureSentinels(
        std::span<RuntimeAssetAnimationTrackTargetBindingRecord>(
            animation_target_bindings.data(),
            animation_target_bindings.size()));
    SeedRuntimeInstanceMappingFailureSentinels(
        std::span<RuntimeAssetRuntimeInstanceMappingRecord>(
            runtime_instance_mappings.data(),
            runtime_instance_mappings.size()));
    ...
    const bool request_animation_target_binding_output = animation_target_binding_capacity != 0U;
    const bool request_runtime_instance_mapping_output = runtime_instance_mapping_capacity != 0U;
    ...
    if (request_runtime_instance_mapping_output) {
        load_request.runtime_instance_mappings = runtime_instance_mappings.data();
        load_request.runtime_instance_mapping_capacity = runtime_instance_mapping_capacity;
    }
    ...
    if (request_animation_target_binding_output) {
        load_request.animation_target_bindings = animation_target_bindings.data();
        load_request.animation_target_binding_capacity = animation_target_binding_capacity;
    }
    ...
    if (request_animation_target_binding_output &&
        !AnimationTargetBindingFailureSentinelsUnchanged(
            std::span<const RuntimeAssetAnimationTrackTargetBindingRecord>(
                animation_target_bindings.data(),
                animation_target_bindings.size()))) {
        return false;
    }
    ...
    if (request_runtime_instance_mapping_output &&
        !RuntimeInstanceMappingFailureSentinelsUnchanged(
...
int RuntimeAssetDataGenericRenderSceneSubmissionBuildsPerEntityMaterialTableFromRefs() {
    MountTable table;
    ResourceRegistry registry;
    AssetManager manager;
    LoadedGraph graph{};
    ...
    RuntimeAssetRenderSceneMaterialBinding material_binding{};
    if (!LoadGenericRenderSceneSubmissionInputs(
            "GenericRenderSceneSubmissionMaterialTable",
            &table,
            &registry,
            &manager,
            &graph,
            ...
            &geometry_bindings,
            &material_binding)) {
        return Fail("generic render scene submission material table inputs failed");
    }
    ...
    if (status != RuntimeAssetDataStatus::Success ||
        result.status != RuntimeAssetDataStatus::Success ||
        result.frame_status != RenderSceneRuntimeFrameStatus::Success) {
        return Fail("generic render scene submission material table build failed");
    }

    if (result.material_variant_count != 2U ||
        result.material_table_count != 2U ||
        result.resolved_material_count != 2U ||
        result.frame_result.material_count != 2U ||
        result.frame_result.material_variant_count != 2U) {
        return Fail("generic render scene submission material table counts changed");
    }

    if (frame_entities[0U].material_table_index != 0U ||
        frame_entities[1U].material_table_index != 1U ||
        frame_entities[2U].material_table_index != 0U) {
        return Fail("generic render scene submission material table indexes changed");
    }

    if (frame_materials[0U].material_id != material.material_id ||
        frame_materials[1U].material_id != variant_material_binding.material.material_id ||
        draws[0U].draw.material_id != material.material_id ||
        draws[1U].draw.material_id != variant_material_binding.material.material_id ||
        draws[2U].draw.material_id != material.material_id) {
        return Fail("generic render scene submission material table routing changed");
    }

    if (frame_materials[1U].material_constant_byte_count !=
        variant_material_binding.material.material_constant_byte_count) {
        return Fail("generic render scene submission material constants count changed");
    }

    if (frame_materials[1U].material_constant_byte_count > 0U &&
        frame_materials[1U].material_constant_bytes[0U] !=
        variant_material_binding.material.material_constant_bytes[0U]) {
        return Fail("generic render scene submission material constants bytes changed");
    }
...
int RuntimeAssetDataGenericRenderSceneSubmissionUsesMaterialRefOrderNotEntityOrder() {
    ...
    RuntimeAssetRenderSceneMaterialBinding material_binding{};
    if (!LoadGenericRenderSceneSubmissionInputs(
            "GenericRenderSceneSubmissionMaterialTableOrder",
            &table,
            &registry,
            &manager,
            &graph,
            ...
            &geometry_bindings,
            &material_binding)) {
        return Fail("generic render scene submission material table order inputs failed");
    }

    std::array<RuntimeAssetSceneEntityRecord, 3U> scene_entities = graph.scene_entities;
    const std::uint32_t base_material_ref_index = graph.scene_entities[0U].material_ref_index;
    const std::uint32_t variant_material_ref_index = base_material_ref_index + 100U;
    scene_entities[0U].material_ref_index = variant_material_ref_index;
    scene_entities[1U].material_ref_index = base_material_ref_index;
    scene_entities[2U].material_ref_index = variant_material_ref_index;
    RuntimeAssetRenderSceneMaterialBinding variant_material_binding{};
    variant_material_binding.resource_ref_index = variant_material_ref_index;
    variant_material_binding.material =
        MakeGenericRenderSceneSubmissionVariantMaterial(material, material.material_id + 100U);
    std::array<RuntimeAssetRenderSceneMaterialBinding, 2U> reversed_material_bindings{
        variant_material_binding,
        material_binding};
    std::array<RenderSceneRuntimeFrameEntityRequest, 3U> frame_entities{};
    std::array<RenderSceneRuntimeMaterialRecord, 2U> frame_materials{};
    ...
    RuntimeAssetRenderSceneSubmissionRequest request = BuildGenericRenderSceneSubmissionRequest(
        ...
        std::span<const RuntimeAssetSceneTransformOutputRecord>(graph.scene_transforms.data(), graph.scene_transforms.size()),
        std::span<const RuntimeAssetRenderSceneGeometryBinding>(geometry_bindings.data(), geometry_bindings.size()),
        std::span<const RuntimeAssetRenderSceneMaterialBinding>(
            reversed_material_bindings.data(),
            reversed_material_bindings.size()),
        camera,
        std::span<RenderSceneRuntimeFrameEntityRequest>(frame_entities.data(), frame_entities.size()),
        std::span<RenderSceneRuntimeFrameDrawRecord>(draws.data(), draws.size()));
    request.require_shared_material = false;
    ...
    if (status != RuntimeAssetDataStatus::Success ||
        result.frame_status != RenderSceneRuntimeFrameStatus::Success) {
        return Fail("generic render scene submission material table order build failed");
    }

    if (frame_materials[0U].material_id != material.material_id ||
        frame_materials[1U].material_id != variant_material_binding.material.material_id) {
        return Fail("generic render scene submission material table used binding or entity order");
    }

    if (frame_entities[0U].material_table_index != 1U ||
        frame_entities[1U].material_table_index != 0U ||
        frame_entities[2U].material_table_index != 1U) {
        return Fail("generic render scene submission material ref indexes changed");
    }

    if (draws[0U].draw.material_id != variant_material_binding.material.material_id ||
        draws[1U].draw.material_id != material.material_id ||
        draws[2U].draw.material_id != variant_material_binding.material.material_id) {
        return Fail("generic render scene submission material ref draw routing changed");
    }

    return 0;
}

int RuntimeAssetDataGenericRenderSceneSubmissionRejectsSmallMaterialTableWithoutMutation() {
    MountTable table;
    ResourceRegistry registry;
    AssetManager manager;
    LoadedGraph graph{};
    ...
    RuntimeAssetRenderSceneMaterialBinding material_binding{};
    if (!LoadGenericRenderSceneSubmissionInputs(
            "GenericRenderSceneSubmissionMaterialTableCapacity",
            &table,
            &registry,
            &manager,
            &graph,
            ...
            &geometry_bindings,
            &material_binding)) {
        return Fail("generic render scene submission material table capacity inputs failed");
    }

    std::array<RuntimeAssetSceneEntityRecord, 3U> scene_entities = graph.scene_entities;
    const std::uint32_t variant_material_ref_index =
        graph.scene_entities[0U].material_ref_index + 100U;
    scene_entities[1U].material_ref_index = variant_material_ref_index;
    RuntimeAssetRenderSceneMaterialBinding variant_material_binding{};
    variant_material_binding.resource_ref_index = variant_material_ref_index;
    variant_material_binding.material =
    ...
    if (status != RuntimeAssetDataStatus::CapacityExceeded ||
        ...
        result.first_missing_resource_ref_index != variant_material_ref_index ||
        result.material_variant_count != 2U ||
        result.frame_status != RenderSceneRuntimeFrameStatus::OutputCapacityExceeded) {
        return Fail("generic render scene submission material table capacity diagnostics changed");
    }

    if (!GenericRenderSceneSubmissionSentinelsUnchanged(
            std::span<const RenderSceneRuntimeFrameEntityRequest>(frame_entities.data(), frame_entities.size()),
            std::span<const RenderSceneRuntimeFrameDrawRecord>(draws.data(), draws.size()))) {
        return Fail("generic render scene submission material table capacity mutated frame outputs");
    }

    if (!GenericRenderSceneSubmissionMaterialSentinelsUnchanged(
            std::span<const RenderSceneRuntimeMaterialRecord>(frame_materials.data(), frame_materials.size()))) {
        return Fail("generic render scene submission material table capacity mutated material outputs");
    }

    return 0;
}

int RuntimeAssetDataGenericRenderSceneSubmissionRejectsDuplicateMaterialIdWithoutMutation() {
    MountTable table;
    ResourceRegistry registry;
    AssetManager manager;
    LoadedGraph graph{};
    ...
    RuntimeAssetRenderSceneMaterialBinding material_binding{};
    if (!LoadGenericRenderSceneSubmissionInputs(
            "GenericRenderSceneSubmissionDuplicateMaterialId",
            &table,
            &registry,
            &manager,
            &graph,
            ...
            &geometry_bindings,
            &material_binding)) {
        return Fail("generic render scene submission duplicate material id inputs failed");
    }
    ...
    RuntimeAssetRenderSceneSubmissionResult result{};
    const RuntimeAssetDataStatus status = BuildRuntimeAssetRenderSceneSubmission(request, &result);
    if (status != RuntimeAssetDataStatus::DuplicateDependency ||
        result.status != RuntimeAssetDataStatus::DuplicateDependency ||
        result.frame_status != RenderSceneRuntimeFrameStatus::DuplicateMaterialRecord ||
        result.first_failed_entity_index != 1U ||
        result.first_missing_resource_ref_index != variant_material_ref_index ||
        result.material_variant_count != 2U ||
        result.material_table_count != 2U ||
        result.submitted_entity_count != 0U ||
        result.frame_result.output_draw_count != 0U) {
        return Fail("generic render scene submission duplicate material id diagnostics changed");
    }

    if (!GenericRenderSceneSubmissionSentinelsUnchanged(
            std::span<const RenderSceneRuntimeFrameEntityRequest>(frame_entities.data(), frame_entities.size()),
            std::span<const RenderSceneRuntimeFrameDrawRecord>(draws.data(), draws.size()))) {
        return Fail("generic render scene submission duplicate material id mutated frame outputs");
    }

    if (!GenericRenderSceneSubmissionMaterialSentinelsUnchanged(
            std::span<const RenderSceneRuntimeMaterialRecord>(frame_materials.data(), frame_materials.size()))) {
        return Fail("generic render scene submission duplicate material id mutated material outputs");
    }
...
int RuntimeAssetDataRuntimeInstanceMappingBuildsTargetFamilyRows() {
    MountTable table;
    if (!CreateMountedTable(TestRoot("BoundedRuntimeInstanceMapping"), &table)) {
        return Fail("mount setup failed");
    }
    ...
    if (!BoundedRuntimeInstanceMappingOutputTableMatches(graph)) {
        return Fail("bounded runtime instance mapping target family table changed");
    }

    return 0;
}

int RuntimeAssetDataRuntimeInstanceMappingFeedsAnimationTargetBinding() {
    MountTable table;
    if (!CreateMountedTable(TestRoot("BoundedRuntimeInstanceMappingBinding"), &table)) {
        return Fail("mount setup failed");
    }
    ...
    BoundedLoadedGraph graph{};
    if (!LoadBoundedRuntimeAssetRecords(table, registry, manager, &graph)) {
        return Fail("bounded target binding runtime asset records failed");
    }

    if (!BoundedRuntimeInstanceMappingOutputTableMatches(graph)) {
        return Fail("bounded runtime instance mapping table changed");
    }

    if (!BoundedAnimationTargetBindingOutputTableMatches(graph)) {
        return Fail("bounded animation target binding table changed");
    }

    if (!Approx(graph.scene_entities[2U].transform.rotation_y, 0.5F) ||
        !Approx(graph.scene_entities[2U].transform.translation_y, 2.0F)) {
        return Fail("bounded runtime instance mapping did not feed target binding");
    }
...
int RuntimeAssetDataRuntimeInstanceMappingRejectsMissingSceneEntityWithoutMutation() {
    ...
    if (!ProbeBoundedFailureCase(
            "BoundedRuntimeInstanceMappingMissingEntityNoMutation",
            scene,
            BoundedAnimationBytes(),
            RuntimeAssetDataStatus::MissingDependency,
            3U,
            0U,
            3U)) {
        return Fail("bounded runtime instance mapping missing entity failure mutated outputs");
    }

    return 0;
}

int RuntimeAssetDataRuntimeInstanceMappingRejectsCapacityOverflowWithoutMutation() {
    if (!ProbeBoundedFailureCase(
            "BoundedRuntimeInstanceMappingCapacityNoMutation",
            BoundedSceneBytes(),
            BoundedAnimationBytes(),
            RuntimeAssetDataStatus::CapacityExceeded,
            3U,
            0U,
            2U)) {
        return Fail("bounded runtime instance mapping capacity failure mutated outputs");
    }

    return 0;
}

int RuntimeAssetDataRuntimeInstanceMappingResolvesModelAndSkeletonFamilies() {
    MountTable table;
    if (!CreateMountedTable(TestRoot("BoundedRuntimeInstanceMappingTargetFamilies"), &table)) {
        return Fail("mount setup failed");
    }
    ...
    BoundedLoadedGraph graph{};
    if (!LoadBoundedRuntimeAssetRecords(table, registry, manager, &graph)) {
        return Fail("bounded runtime instance mapping target family records failed");
    }

    if (!BoundedRuntimeInstanceMappingOutputTableMatches(graph)) {
        return Fail("bounded runtime instance mapping target families changed");
    }

    return 0;
}

int RuntimeAssetDataAnimationTrackTargetBindingResolvesTargetIdAndProperty() {
    MountTable table;
    if (!CreateMountedTable(TestRoot("BoundedAnimationTargetBinding"), &table)) {
        return Fail("mount setup failed");
    }
    ...
    if (graph.scene_output.animation_sampled_value_count != 2U ||
        !Approx(graph.scene_entities[2U].transform.rotation_y, 0.5F) ||
        !Approx(graph.scene_entities[2U].transform.translation_y, 2.0F)) {
        return Fail("bounded target binding did not update expected scene target");
    }

    return 0;
}

int RuntimeAssetDataAnimationTrackTargetBindingRejectsMissingTargetWithoutMutation() {
    const std::string animation =
        ReplaceFirst(BoundedTargetBindingAnimationBytes(), "target_id=1001", "target_id=9001");
    if (!ProbeBoundedFailureCase(
            "BoundedAnimationTargetBindingMissingNoMutation",
            BoundedSceneBytes(),
            animation,
            RuntimeAssetDataStatus::MissingDependency,
            3U,
            3U)) {
        return Fail("bounded missing animation target binding failure mutated outputs");
    }

    return 0;
}

int RuntimeAssetDataAnimationTrackTargetBindingRejectsUnsupportedPropertyWithoutMutation() {
    const std::string animation =
        ReplaceFirst(BoundedTargetBindingAnimationBytes(), "property=transform:rotation_y", "property=material:opacity");
    if (!ProbeBoundedFailureCase(
            "BoundedAnimationTargetBindingUnsupportedNoMutation",
            BoundedSceneBytes(),
            animation,
            RuntimeAssetDataStatus::UnsupportedFieldValue,
            3U,
            3U)) {
        return Fail("bounded unsupported animation target property failure mutated outputs");
    }

    return 0;
}

int RuntimeAssetDataAnimationTrackTargetBindingRejectsCapacityOverflowWithoutMutation() {
    if (!ProbeBoundedFailureCase(
            "BoundedAnimationTargetBindingCapacityNoMutation",
            BoundedSceneBytes(),
            BoundedTargetBindingAnimationBytes(),
            RuntimeAssetDataStatus::CapacityExceeded,
            3U,
            1U)) {
        return Fail("bounded animation target binding capacity failure mutated outputs");
    }

    return 0;
}

int RuntimeAssetDataAnimationTrackTargetBindingResolvesModelAndSkeletonTargetFamilies() {
    std::string animation =
        ReplaceFirst(BoundedTargetBindingAnimationBytes(), "target_id=1001", "target_id=2001");
    animation = ReplaceFirst(animation, "target_id=1001", "target_id=3001");

    MountTable table;
    if (!CreateMountedTable(TestRoot("BoundedAnimationTargetBindingTargetFamilies"), &table)) {
        return Fail("mount setup failed");
    }
    ...
    if (!WriteBytes(table, "Animation/Spin.yuanim", BytesFromString(animation))) {
        return Fail("bounded target family animation write failed");
    }
    ...
    BoundedLoadedGraph graph{};
    if (!LoadBoundedRuntimeAssetRecords(table, registry, manager, &graph)) {
        return Fail("bounded animation target family records failed");
    }

    if (!BoundedRuntimeInstanceMappingOutputTableMatches(graph)) {
        return Fail("bounded target family runtime instance mapping changed");
    }

    if (!BoundedAnimationTargetBindingOutputTableMatchesTargetIds(graph, 2001U, 3001U)) {
        return Fail("bounded model skeleton target binding table changed");
    }

    if (graph.scene_output.animation_sampled_value_count != 2U ||
        !Approx(graph.scene_entities[2U].transform.rotation_y, 0.5F) ||
        !Approx(graph.scene_entities[2U].transform.translation_y, 2.0F)) {
        return Fail("bounded model skeleton target binding did not update scene target");
    }
...
int RuntimeAssetDataAssetDependencyBatchUsesExplicitRowsAndReportsFailedIndex() {
    ResourceRegistry registry;
    ResourceDescriptor document_resource_desc{};
    document_resource_desc.type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    document_resource_desc.logical_key = ResourceLogicalKey("runtime_asset_batch_document");
    ...
    dependency_a_resource_desc.logical_key = ResourceLogicalKey("runtime_asset_batch_dependency_a");
    ...
    dependency_b_resource_desc.logical_key = ResourceLogicalKey("runtime_asset_batch_dependency_b");
    ...
    if (!document_resource.Succeeded() ||
        ...
        return Fail("runtime asset dependency batch resource registration failed");
    }

    AssetManager manager;
    AssetDescriptor document_asset_desc{};
    document_asset_desc.stable_id = 8501U;
    ...
    dependency_a_asset_desc.stable_id = 8502U;
    ...
    dependency_b_asset_desc.stable_id = 8503U;
    ...
    if (!document_asset.Succeeded() ||
        ...
        return Fail("runtime asset dependency batch asset registration failed");
    }

    std::array<RuntimeAssetDataAssetDependencyRecord, 3U> records{};
    records[0U].stable_resource_id = 8502U;
    ...
    records[1U].stable_resource_id = 8503U;
    records[1U].dependent_asset = document_asset.handle;
    records[1U].dependency_asset = AssetHandle{};
    ...
    records[2U].stable_resource_id = 8503U;
    records[2U].dependent_asset = document_asset.handle;
    records[2U].dependency_asset = dependency_b_asset.handle;
    records[2U].expected_resource = dependency_b_resource.handle;
    records[2U].expected_resource_type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};

    RuntimeAssetDataAssetDependencyBatchResult failed_batch{};
    const std::uint32_t record_count = static_cast<std::uint32_t>(records.size());
    const RuntimeAssetDataStatus failed_status =
        CommitRuntimeAssetDataAssetDependencyBatch(
            &manager,
            records.data(),
            record_count,
            &failed_batch);
    if (failed_status != RuntimeAssetDataStatus::AssetDependencyFailed ||
        failed_batch.status != RuntimeAssetDataStatus::AssetDependencyFailed ||
        failed_batch.asset_status != AssetStatus::InvalidHandle) {
        return Fail("runtime asset dependency batch failure status mismatch");
    }

    if (failed_batch.committed_dependency_edge_count != 1U ||
        failed_batch.first_failed_dependency_index != 1U) {
        return Fail("runtime asset dependency batch failure counts mismatch");
    }

    AssetSnapshot snapshot = manager.Snapshot();
    if (snapshot.active_dependency_edge_count != 1U) {
        return Fail("runtime asset dependency batch committed wrong edge count");
    }

    std::array<AssetHandle, 2U> dependencies{};
    std::uint32_t dependency_count = 0U;
    AssetStatus traversal_status =
        manager.TraverseDependencies(document_asset.handle, dependencies.data(), 2U, &dependency_count);
    if (traversal_status != AssetStatus::Success) {
        return Fail("runtime asset dependency batch traversal after failure failed");
    }

    if (dependency_count != 1U || dependencies[0U].slot != dependency_a_asset.handle.slot) {
        return Fail("runtime asset dependency batch first committed dependency mismatch");
    }

    RuntimeAssetDataAssetDependencyBatchResult resume_batch{};
    const RuntimeAssetDataStatus resume_status =
        CommitRuntimeAssetDataAssetDependencyBatch(
            &manager,
            &records[2U],
            1U,
            &resume_batch);
    if (resume_status != RuntimeAssetDataStatus::Success ||
        resume_batch.status != RuntimeAssetDataStatus::Success ||
        resume_batch.asset_status != AssetStatus::Success) {
        return Fail("runtime asset dependency batch resume status mismatch");
    }

    if (resume_batch.committed_dependency_edge_count != 1U ||
        resume_batch.first_failed_dependency_index != 0U) {
        return Fail("runtime asset dependency batch resume counts mismatch");
    }

    snapshot = manager.Snapshot();
    if (snapshot.active_dependency_edge_count != 2U) {
        return Fail("runtime asset dependency batch resume committed wrong edge count");
    }
    ...
    traversal_status =
        manager.TraverseDependencies(document_asset.handle, dependencies.data(), 2U, &dependency_count);
    if (traversal_status != AssetStatus::Success) {
        return Fail("runtime asset dependency batch final traversal failed");
    }

    if (dependency_count != 2U) {
        return Fail("runtime asset dependency batch final traversal count mismatch");
    }

    return 0;
}

int RuntimeAssetDataWorldSceneAuthoringAssetDependencyBatchUsesExportRows() {
    ResourceRegistry registry;
    ResourceDescriptor document_resource_desc{};
    document_resource_desc.type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    document_resource_desc.logical_key = ResourceLogicalKey("runtime_asset_authoring_batch_document");
    const ResourceRegistrationResult document_resource =
        registry.RegisterSyntheticDescriptor(document_resource_desc);

    ResourceDescriptor dependency_a_resource_desc{};
    dependency_a_resource_desc.type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    dependency_a_resource_desc.logical_key = ResourceLogicalKey("runtime_asset_authoring_batch_dependency_a");
    const ResourceRegistrationResult dependency_a_resource =
        registry.RegisterSyntheticDescriptor(dependency_a_resource_desc);

    ResourceDescriptor dependency_b_resource_desc{};
    dependency_b_resource_desc.type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    dependency_b_resource_desc.logical_key = ResourceLogicalKey("runtime_asset_authoring_batch_dependency_b");
    const ResourceRegistrationResult dependency_b_resource =
        registry.RegisterSyntheticDescriptor(dependency_b_resource_desc);

    ResourceDescriptor dependency_c_resource_desc{};
    dependency_c_resource_desc.type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    dependency_c_resource_desc.logical_key = ResourceLogicalKey("runtime_asset_authoring_batch_dependency_c");
    ...
    if (!document_resource.Succeeded() ||
        ...
        return Fail("runtime asset authoring batch resource registration failed");
    }

    AssetManager manager;
    AssetDescriptor document_asset_desc{};
    document_asset_desc.stable_id = 8601U;
    document_asset_desc.asset_type = AssetTypeId{ASSET_TYPE_TEXTURE};
    document_asset_desc.resource = document_resource.handle;
    document_asset_desc.resource_type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    const AssetRegistrationResult document_asset =
    ...
    AssetDescriptor dependency_a_asset_desc{};
    dependency_a_asset_desc.stable_id = 8602U;
    dependency_a_asset_desc.asset_type = AssetTypeId{ASSET_TYPE_TEXTURE};
    dependency_a_asset_desc.resource = dependency_a_resource.handle;
    dependency_a_asset_desc.resource_type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    const AssetRegistrationResult dependency_a_asset =
    ...
    AssetDescriptor dependency_b_asset_desc{};
    dependency_b_asset_desc.stable_id = 8603U;
    dependency_b_asset_desc.asset_type = AssetTypeId{ASSET_TYPE_TEXTURE};
    dependency_b_asset_desc.resource = dependency_b_resource.handle;
    dependency_b_asset_desc.resource_type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    const AssetRegistrationResult dependency_b_asset =
    ...
    dependency_c_asset_desc.stable_id = 8604U;
    ...
    if (!document_asset.Succeeded() ||
        ...
        return Fail("runtime asset authoring batch asset registration failed");
    }

    std::array<WorldSceneAuthoringDependencyRecord, 3U> authoring_dependencies{};
    authoring_dependencies[0U].stable_resource_id = 8602U;
    ...
    authoring_dependencies[1U].stable_resource_id = 8603U;
    ...
    authoring_dependencies[2U].stable_resource_id = 8604U;
    ...
    if (failed_status != RuntimeAssetDataStatus::AssetDependencyFailed ||
        failed_batch.status != RuntimeAssetDataStatus::AssetDependencyFailed ||
        failed_batch.asset_status != AssetStatus::InvalidHandle) {
        return Fail("runtime asset authoring batch failure status mismatch");
    }

    if (failed_batch.committed_dependency_edge_count != 2U ||
        failed_batch.first_failed_dependency_index != 2U) {
        return Fail("runtime asset authoring batch failure counts mismatch");
    }

    AssetSnapshot snapshot = manager.Snapshot();
    if (snapshot.active_dependency_edge_count != 2U) {
        return Fail("runtime asset authoring batch committed wrong edge count");
    }

    std::array<AssetHandle, 3U> dependencies{};
    std::uint32_t traversed_dependency_count = 0U;
    AssetStatus traversal_status =
        manager.TraverseDependencies(document_asset.handle, dependencies.data(), 3U, &traversed_dependency_count);
    if (traversal_status != AssetStatus::Success) {
        return Fail("runtime asset authoring batch traversal after failure failed");
    }

    if (traversed_dependency_count != 2U) {
        return Fail("runtime asset authoring batch traversal count mismatch");
    }

    if (dependencies[0U].slot != dependency_a_asset.handle.slot ||
        dependencies[1U].slot != dependency_b_asset.handle.slot) {
        return Fail("runtime asset authoring batch traversal handle mismatch");
    }

    std::uint32_t resume_dependency_count = 1U;
    WorldSceneAuthoringRuntimeExport resume_export{};
    resume_export.dependency_records = &authoring_dependencies[2U];
    resume_export.dependency_capacity = resume_dependency_count;
    resume_export.dependency_count = &resume_dependency_count;
    const AssetHandle resume_dependency_asset = dependency_c_asset.handle;
    request.runtime_export = &resume_export;
    request.dependency_assets = &resume_dependency_asset;
    request.dependency_asset_count = 1U;
    ...
    if (resume_status != RuntimeAssetDataStatus::Success ||
        resume_batch.status != RuntimeAssetDataStatus::Success ||
        resume_batch.asset_status != AssetStatus::Success) {
        return Fail("runtime asset authoring batch resume status mismatch");
    }

    if (resume_batch.committed_dependency_edge_count != 1U ||
        resume_batch.first_failed_dependency_index != 0U) {
        return Fail("runtime asset authoring batch resume counts mismatch");
    }

    snapshot = manager.Snapshot();
    if (snapshot.active_dependency_edge_count != 3U) {
        return Fail("runtime asset authoring batch final edge count mismatch");
    }

    traversed_dependency_count = 0U;
    traversal_status =
        manager.TraverseDependencies(document_asset.handle, dependencies.data(), 3U, &traversed_dependency_count);
    if (traversal_status != AssetStatus::Success) {
        return Fail("runtime asset authoring batch final traversal failed");
    }

    if (traversed_dependency_count != 3U) {
        return Fail("runtime asset authoring batch final traversal count mismatch");
    }

    if (dependencies[2U].slot != dependency_c_asset.handle.slot) {
        return Fail("runtime asset authoring batch final traversal handle mismatch");
    }
...
int RuntimeAssetDataAssetDependencyBatchTypeEnumerationIsAtomic() {
    ResourceRegistry registry;
    ResourceDescriptor document_resource_desc{};
    document_resource_desc.type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    document_resource_desc.logical_key = ResourceLogicalKey("runtime_asset_batch_type_enum_document");
    const ResourceRegistrationResult document_resource =
        registry.RegisterSyntheticDescriptor(document_resource_desc);

    ResourceDescriptor texture_a_resource_desc{};
    texture_a_resource_desc.type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    texture_a_resource_desc.logical_key = ResourceLogicalKey("runtime_asset_batch_type_enum_texture_a");
    const ResourceRegistrationResult texture_a_resource =
        registry.RegisterSyntheticDescriptor(texture_a_resource_desc);

    ResourceDescriptor shader_resource_desc{};
    shader_resource_desc.type = ResourceTypeId{RESOURCE_TYPE_SHADER};
    shader_resource_desc.logical_key = ResourceLogicalKey("runtime_asset_batch_type_enum_shader");
    const ResourceRegistrationResult shader_resource =
        registry.RegisterSyntheticDescriptor(shader_resource_desc);

    ResourceDescriptor texture_c_resource_desc{};
    texture_c_resource_desc.type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    texture_c_resource_desc.logical_key = ResourceLogicalKey("runtime_asset_batch_type_enum_texture_c");
    const ResourceRegistrationResult texture_c_resource =
        registry.RegisterSyntheticDescriptor(texture_c_resource_desc);

    ResourceDescriptor transitive_resource_desc{};
    transitive_resource_desc.type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    transitive_resource_desc.logical_key = ResourceLogicalKey("runtime_asset_batch_type_enum_transitive");
    const ResourceRegistrationResult transitive_resource =
        registry.RegisterSyntheticDescriptor(transitive_resource_desc);
    if (!document_resource.Succeeded() ||
        !texture_a_resource.Succeeded() ||
        ...
        !texture_c_resource.Succeeded() ||
        !transitive_resource.Succeeded()) {
        return Fail("runtime asset batch type enumeration resource registration failed");
    }

    AssetManager manager;
    AssetDescriptor document_asset_desc{};
    document_asset_desc.stable_id = 9101U;
    document_asset_desc.asset_type = AssetTypeId{ASSET_TYPE_TEXTURE};
    document_asset_desc.resource = document_resource.handle;
    document_asset_desc.resource_type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    const AssetRegistrationResult document_asset =
    ...
    AssetDescriptor texture_a_asset_desc{};
    texture_a_asset_desc.stable_id = 9102U;
    texture_a_asset_desc.asset_type = AssetTypeId{ASSET_TYPE_TEXTURE};
    texture_a_asset_desc.resource = texture_a_resource.handle;
    texture_a_asset_desc.resource_type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    const AssetRegistrationResult texture_a_asset =
    ...
    AssetDescriptor shader_asset_desc{};
    shader_asset_desc.stable_id = 9103U;
    shader_asset_desc.asset_type = AssetTypeId{ASSET_TYPE_SHADER};
    shader_asset_desc.resource = shader_resource.handle;
    shader_asset_desc.resource_type = ResourceTypeId{RESOURCE_TYPE_SHADER};
    const AssetRegistrationResult shader_asset =
    ...
    AssetDescriptor texture_c_asset_desc{};
    texture_c_asset_desc.stable_id = 9104U;
    texture_c_asset_desc.asset_type = AssetTypeId{ASSET_TYPE_TEXTURE};
    texture_c_asset_desc.resource = texture_c_resource.handle;
    texture_c_asset_desc.resource_type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    const AssetRegistrationResult texture_c_asset =
    ...
    AssetDescriptor transitive_asset_desc{};
    transitive_asset_desc.stable_id = 9105U;
    transitive_asset_desc.asset_type = AssetTypeId{ASSET_TYPE_TEXTURE};
    transitive_asset_desc.resource = transitive_resource.handle;
    transitive_asset_desc.resource_type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    const AssetRegistrationResult transitive_asset =
    ...
    if (!document_asset.Succeeded() ||
        ...
        !texture_c_asset.Succeeded() ||
        !transitive_asset.Succeeded()) {
        return Fail("runtime asset batch type enumeration asset registration failed");
    }

    std::array<RuntimeAssetDataAssetDependencyRecord, 4U> records{};
    records[0U].stable_resource_id = 9102U;
    records[0U].dependent_asset = document_asset.handle;
    records[0U].dependency_asset = texture_a_asset.handle;
    records[0U].expected_resource = texture_a_resource.handle;
    records[0U].expected_resource_type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    records[1U].stable_resource_id = 9103U;
    records[1U].dependent_asset = document_asset.handle;
    records[1U].dependency_asset = shader_asset.handle;
    records[1U].expected_resource = shader_resource.handle;
    records[1U].expected_resource_type = ResourceTypeId{RESOURCE_TYPE_SHADER};
    records[2U].stable_resource_id = 9104U;
    records[2U].dependent_asset = document_asset.handle;
    records[2U].dependency_asset = texture_c_asset.handle;
    records[2U].expected_resource = texture_c_resource.handle;
    records[2U].expected_resource_type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    records[3U].stable_resource_id = 9105U;
    records[3U].dependent_asset = texture_a_asset.handle;
    records[3U].dependency_asset = transitive_asset.handle;
    records[3U].expected_resource = transitive_resource.handle;
    records[3U].expected_resource_type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};
    ...
    if (batch_status != RuntimeAssetDataStatus::Success ||
        batch_result.status != RuntimeAssetDataStatus::Success ||
        batch_result.committed_dependency_edge_count != 4U) {
        return Fail("runtime asset batch type enumeration commit failed");
    }

    RuntimeAssetDataAssetDependencyTypeEnumerationRequest shader_request{};
    shader_request.asset_manager = &manager;
    shader_request.dependent_asset = document_asset.handle;
    shader_request.expected_resource_type = ResourceTypeId{RESOURCE_TYPE_SHADER};

    RuntimeAssetDataAssetDependencyTypeEnumerationRequest texture_request{};
    texture_request.asset_manager = &manager;
    texture_request.dependent_asset = document_asset.handle;
    texture_request.expected_resource_type = ResourceTypeId{RESOURCE_TYPE_TEXTURE};

    std::array<RuntimeAssetDataAssetDependencyRecord, 1U> shader_records{};
    std::uint32_t shader_count = 0U;
    RuntimeAssetDataAssetDependencyTypeEnumerationResult shader_result{};
    RuntimeAssetDataStatus enum_status =
        EnumerateRuntimeAssetDataAssetDependenciesByType(
            shader_request,
            shader_records.data(),
            static_cast<std::uint32_t>(shader_records.size()),
            &shader_count,
            &shader_result);
    if (enum_status != RuntimeAssetDataStatus::Success ||
        shader_result.status != RuntimeAssetDataStatus::Success ||
        shader_count != 1U) {
        return Fail("runtime asset batch type enumeration shader baseline failed");
    }

    std::array<RuntimeAssetDataAssetDependencyRecord, 2U> texture_records{};
    std::uint32_t texture_count = 0U;
    RuntimeAssetDataAssetDependencyTypeEnumerationResult texture_result{};
    enum_status =
        EnumerateRuntimeAssetDataAssetDependenciesByType(
            texture_request,
            texture_records.data(),
            static_cast<std::uint32_t>(texture_records.size()),
            &texture_count,
            &texture_result);
    if (enum_status != RuntimeAssetDataStatus::Success ||
        texture_result.status != RuntimeAssetDataStatus::Success ||
        texture_count != 2U) {
        return Fail("runtime asset batch type enumeration texture baseline failed");
    }

    RuntimeAssetDataAssetDependencyRecord sentinel_a{};
    sentinel_a.stable_resource_id = 9912U;
    sentinel_a.dependent_asset = AssetHandle{221U, 222U};
    sentinel_a.dependency_asset = AssetHandle{223U, 224U};
    sentinel_a.expected_resource = ResourceHandle{225U, 226U};
    sentinel_a.expected_resource_type = ResourceTypeId{227U};
    RuntimeAssetDataAssetDependencyRecord sentinel_b = sentinel_a;
    sentinel_b.stable_resource_id = 9913U;
    RuntimeAssetDataAssetDependencyRecord sentinel_c = sentinel_a;
    sentinel_c.stable_resource_id = 9914U;
    std::array<RuntimeAssetDataAssetDependencyRecord, 4U> output_records{
        sentinel_a,
        sentinel_b,
        sentinel_c,
        sentinel_a};
    ...
    std::array<RuntimeAssetDataAssetDependencyTypeEnumerationRequest, 2U> requests{};
    requests[0U] = shader_request;
    requests[1U] = texture_request;
    RuntimeAssetDataAssetDependencyBatchTypeEnumerationRequest batch_request{};
    batch_request.requests = requests.data();
    batch_request.request_count = static_cast<std::uint32_t>(requests.size());

    RuntimeAssetDataAssetDependencyBatchTypeEnumerationResult capacity_result{};
    RuntimeAssetDataStatus batch_enum_status =
        EnumerateRuntimeAssetDataAssetDependenciesByTypes(
            batch_request,
            output_records.data(),
            2U,
            &output_count,
            &capacity_result);
    if (batch_enum_status != RuntimeAssetDataStatus::CapacityExceeded ||
        capacity_result.status != RuntimeAssetDataStatus::CapacityExceeded ||
        capacity_result.asset_status != AssetStatus::OutputBufferTooSmall ||
        capacity_result.required_dependency_count != 3U ||
        capacity_result.copied_dependency_count != 0U ||
        output_count != 77U) {
        return Fail("runtime asset batch type enumeration capacity status mismatch");
    }

    if (output_records[0U].stable_resource_id != sentinel_a.stable_resource_id ||
        output_records[1U].stable_resource_id != sentinel_b.stable_resource_id ||
        output_records[2U].stable_resource_id != sentinel_c.stable_resource_id) {
        return Fail("runtime asset batch type enumeration capacity mutated output");
    }

    const AssetSnapshot before_batch_snapshot = manager.Snapshot();
    RuntimeAssetDataAssetDependencyBatchTypeEnumerationResult success_result{};
    batch_enum_status =
        EnumerateRuntimeAssetDataAssetDependenciesByTypes(
            batch_request,
            output_records.data(),
            ...
            &output_count,
            &success_result);
    if (batch_enum_status != RuntimeAssetDataStatus::Success ||
        success_result.status != RuntimeAssetDataStatus::Success ||
        success_result.asset_status != AssetStatus::Success ||
        success_result.required_dependency_count != 3U ||
        success_result.copied_dependency_count != 3U ||
        output_count != 3U) {
        return Fail("runtime asset batch type enumeration success status mismatch");
    }

    if (output_records[0U].stable_resource_id != shader_records[0U].stable_resource_id ||
        output_records[0U].dependency_asset.slot != shader_records[0U].dependency_asset.slot ||
        output_records[1U].stable_resource_id != texture_records[0U].stable_resource_id ||
        output_records[1U].dependency_asset.slot != texture_records[0U].dependency_asset.slot ||
        output_records[2U].stable_resource_id != texture_records[1U].stable_resource_id ||
        output_records[2U].dependency_asset.slot != texture_records[1U].dependency_asset.slot) {
        return Fail("runtime asset batch type enumeration success order mismatch");
    }

    const RuntimeAssetDataAssetDependencyRecord prior_a = output_records[0U];
    const RuntimeAssetDataAssetDependencyRecord prior_b = output_records[1U];
    const RuntimeAssetDataAssetDependencyRecord prior_c = output_records[2U];
    requests[1U].expected_resource_type = ResourceTypeId{};
    output_count = 91U;
    RuntimeAssetDataAssetDependencyBatchTypeEnumerationResult invalid_type_result{};
    batch_enum_status =
        EnumerateRuntimeAssetDataAssetDependenciesByTypes(
            batch_request,
            output_records.data(),
            ...
            &output_count,
            &invalid_type_result);
    if (batch_enum_status != RuntimeAssetDataStatus::InvalidArgument ||
        invalid_type_result.status != RuntimeAssetDataStatus::InvalidArgument ||
        invalid_type_result.asset_status != AssetStatus::InvalidArgument ||
        invalid_type_result.first_failed_request_index != 1U ||
        output_count != 91U) {
        return Fail("runtime asset batch type enumeration invalid type status mismatch");
    }

    if (output_records[0U].stable_resource_id != prior_a.stable_resource_id ||
        output_records[1U].stable_resource_id != prior_b.stable_resource_id ||
        output_records[2U].stable_resource_id != prior_c.stable_resource_id) {
        return Fail("runtime asset batch type enumeration invalid type mutated output");
    }

    requests[1U] = texture_request;
    requests[0U].dependent_asset = AssetHandle{};
    RuntimeAssetDataAssetDependencyBatchTypeEnumerationResult invalid_handle_result{};
    batch_enum_status =
        EnumerateRuntimeAssetDataAssetDependenciesByTypes(
            batch_request,
            output_records.data(),
            ...
            &output_count,
            &invalid_handle_result);
    if (batch_enum_status != RuntimeAssetDataStatus::AssetDependencyFailed ||
        invalid_handle_result.status != RuntimeAssetDataStatus::AssetDependencyFailed ||
        invalid_handle_result.asset_status != AssetStatus::InvalidHandle ||
        invalid_handle_result.first_failed_request_index != 0U) {
        return Fail("runtime asset batch type enumeration invalid handle status mismatch");
    }

    requests[0U] = shader_request;
    requests[1U] = texture_request;
    requests[1U].asset_manager = nullptr;
    RuntimeAssetDataAssetDependencyBatchTypeEnumerationResult invalid_manager_result{};
    batch_enum_status =
        EnumerateRuntimeAssetDataAssetDependenciesByTypes(
            batch_request,
            output_records.data(),
            static_cast<std::uint32_t>(output_records.size()),
            &output_count,
            &invalid_manager_result);
    if (batch_enum_status != RuntimeAssetDataStatus::InvalidArgument ||
        invalid_manager_result.status != RuntimeAssetDataStatus::InvalidArgument ||
        invalid_manager_result.asset_status != AssetStatus::InvalidArgument ||
        invalid_manager_result.first_failed_request_index != 1U) {
        return Fail("runtime asset batch type enumeration invalid manager status mismatch");
    }

    requests[0U] = shader_request;
    requests[1U] = texture_request;
    output_count = 42U;
    RuntimeAssetDataAssetDependencyBatchTypeEnumerationResult repeat_capacity_result{};
    batch_enum_status =
        EnumerateRuntimeAssetDataAssetDependenciesByTypes(
            batch_request,
            output_records.data(),
            2U,
            &output_count,
            &repeat_capacity_result);
    if (batch_enum_status != RuntimeAssetDataStatus::CapacityExceeded ||
        repeat_capacity_result.status != RuntimeAssetDataStatus::CapacityExceeded ||
        repeat_capacity_result.required_dependency_count != 3U ||
        output_count != 42U) {
        return Fail("runtime asset batch type enumeration repeat capacity status mismatch");
    }
    ...
    if (output_records[0U].stable_resource_id != prior_a.stable_resource_id ||
        ...
        output_records[1U].dependency_asset.slot != prior_b.dependency_asset.slot ||
        output_records[2U].stable_resource_id != prior_c.stable_resource_id ||
        output_records[2U].dependency_asset.slot != prior_c.dependency_asset.slot) {
        return Fail("runtime asset batch type enumeration repeat failure mutated output");
    }

    const AssetSnapshot after_batch_snapshot = manager.Snapshot();
    if (after_batch_snapshot.active_dependency_edge_count != before_batch_snapshot.active_dependency_edge_count ||
        after_batch_snapshot.accepted_operation_count != before_batch_snapshot.accepted_operation_count ||
        after_batch_snapshot.failed_operation_count != before_batch_snapshot.failed_operation_count) {
        return Fail("runtime asset batch type enumeration mutated asset manager diagnostics");
    }
...
const std::unordered_map<std::string_view, TestFunction> TESTS = {
    ...
    {TEST_GENERIC_RENDER_SCENE_SUBMISSION_MATERIAL_TABLE_CAPACITY,
     RuntimeAssetDataGenericRenderSceneSubmissionRejectsSmallMaterialTableWithoutMutation},
    {TEST_GENERIC_RENDER_SCENE_SUBMISSION_DUPLICATE_MATERIAL_ID,
     RuntimeAssetDataGenericRenderSceneSubmissionRejectsDuplicateMaterialIdWithoutMutation},
    ...
    {TEST_RUNTIME_INSTANCE_MAPPING_CAPACITY,
     RuntimeAssetDataRuntimeInstanceMappingRejectsCapacityOverflowWithoutMutation},
    {TEST_RUNTIME_INSTANCE_MAPPING_TARGET_FAMILY,
     RuntimeAssetDataRuntimeInstanceMappingResolvesModelAndSkeletonFamilies},
    {TEST_ANIMATION_TARGET_BINDING,
     RuntimeAssetDataAnimationTrackTargetBindingResolvesTargetIdAndProperty},
    {TEST_ANIMATION_TARGET_BINDING_MISSING_TARGET,
     RuntimeAssetDataAnimationTrackTargetBindingRejectsMissingTargetWithoutMutation},
    {TEST_ANIMATION_TARGET_BINDING_UNSUPPORTED_PROPERTY,
    ...
    {TEST_ANIMATION_TARGET_BINDING_TARGET_FAMILY,
     RuntimeAssetDataAnimationTrackTargetBindingResolvesModelAndSkeletonTargetFamilies},
    ...
    {TEST_RUNTIME_ASSET_DEPENDENCY_BATCH,
    ...
    {TEST_RUNTIME_DEPENDENCY_BATCH_TYPE_ENUMERATION,
     RuntimeAssetDataAssetDependencyBatchTypeEnumerationIsAtomic},
