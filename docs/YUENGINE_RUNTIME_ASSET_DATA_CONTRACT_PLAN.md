...
is now VQ-closed at `f967001c39a53717226127ff67c316c8a3bf2a4a`; it proves
exported `WorldSceneAuthoringDependencyRecord.resource_handle` values can be
committed as caller-owned `ResourceRegistry` dependency edges with a
caller-owned dependent `ResourceHandle` and exported dependency
`ResourceHandle`. Implementation task `0c9911ab` and VQ task
`86ce7cfd-627c-445a-ae03-f639d47cec13` report COMPLETE-PASS. Focused build
`YuRuntimeAssetWorldAdapterHandoffTests` PASS; new Resource edge row
...
pass `4/4`. `stable_resource_id`-only inference is rejected, and
RuntimeAssetWorldAdapter restore handoff itself remains non-mutating for
...
proof should explicitly commit caller-owned `AssetHandle` dependency edges,
reject stable-id-only/default-handle inference, and keep RuntimeAssetWorldAdapter
