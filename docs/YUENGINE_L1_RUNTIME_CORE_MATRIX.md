...
| Render scene | `L1-RSCENE-001..006`, `L1-CAMERA-001..003`, `L1-GEOM-001`, `L1-MAT-001` | RenderScene owner owns render entity/camera records, full-list visibility, RenderCore packet assembly, multi-entity runtime visual submission, and failure statuses | Object/transform records, Asset handles, L0 RenderCore/RHI, runtime camera/geometry/material/resource records | RuntimeApp frame context, vertical sample route, runtime visual foundation route | FirstSlice | Existing value-contract rows: `RenderScene_BuildsRenderCoreViewPacket`, `RenderScene_FullListVisibilityOutputsOnlyVisibleEntities`, `RenderScene_Missing*`, `RenderScene_NoNativeOrUpperDependency`; missing visual floors: camera capture, cube/cylinder/cone geometry, material texture slots, and `L1-RSCENE-006` multi-entity captured frame proof | `L1-SAMPLE-006` submits render scene through value contracts; `L1-SAMPLE-011/012` must consume the runtime visual foundation ladder | Target D3D11/display may be `BlockedByEnv`; missing camera, model geometry, material texture slots, shader/pipeline, RenderScene multi-entity, RenderCore multi-draw, capture, or resource resolution is a stage-close runtime blocker | D3D11 includes in public L1 headers, gameplay scene policy, lighting, animation/skinning, UI rendering, editor viewport, standalone D3D sample bypass | Land camera, geometry, material, and multi-entity floors before final visual sample closure |
...
| Animation runtime foundation | `L1-ANIM-001..005` | Animation owner owns bounded runtime clip/track/keyframe records, interpolation sampler, FrameContext time sampling, transform application, and failure states | FrameContext time, Object/Transform records | RenderScene consumes sampled transform values; vertical visual sample may use animation output | StageClose | Future tests must prove deterministic interpolation at fixed times, runtime-time sampling, sampled transform application, and missing/invalid/capacity failure states | `L1-SAMPLE-011` may drive per-object rotation through animation; if not implemented, the sample must report the exact `L1-ANIM-*` blocker | None as environment blocker; missing interpolation is semantic missing foundation | Editor timeline, gameplay state machine, skeletal skinning, blend tree, montage graph, physics, UI animation authoring | Add minimal runtime interpolation before claiming animation-capable visual closure |
...
| L1 vertical sample prep | `L1-SAMPLE-001..012`, `L1-VIS-001..006` | Sample owner aggregates L1 value-contract proof for manifest, runtime boot, object graph, asset/render/audio routes, input, serialize, cleanup, validation, and pure runtime visual scene closure | All L1 subsystem rows above plus L0 explicit hardware statuses and the runtime visual foundation ladder from camera/geometry/material/animation through RenderScene/RenderCore/RHI/capture | RuntimeApp, World, Asset, Input, RenderScene, AudioScene, Serialize, Diagnostics, runtime camera/material/resource/animation records | FirstSlice | Existing value rows: `Sample_L1VerticalPrep_BuildsManifestAndSubmitPrep`, `Sample_L1VerticalPrep_UsesValueContractsWithoutHardware`, `Sample_L1VerticalPrep_DeterministicObjectGraphHasStableSlots`, `Sample_L1VerticalPrep_RoutesAssetRenderAudioAndLifecycle`, `Sample_L1VerticalPrep_ExposesDebugReleaseFastValidationRoute`, `Sample_L1VerticalPrep_RoundTripsStateAndCleansActiveRecords`; missing route: checked-in runtime visual command for `L1-VIS-001..006` and `L1-SAMPLE-011/012` | This is the vertical sample linkage row; visual closure requires the shallow-to-deep foundation sequence plus cube/cylinder/cone full-orbit capture set or exact missing-layer blocker | `docs/YUENGINE_L1_VERTICAL_SAMPLE_ACCEPTANCE.md` and `docs/YUENGINE_RUNTIME_VISUAL_FOUNDATION_PLAN.md` exist; real D3D11/XAudio2/XInput remain L0 env blockers, release/native sample runs remain stage-close evidence, and runtime visual foundation route is not implemented yet | TouhouNewWorld business logic, old package runtime parsing, UI/GameAdapter/gameplay, editor/Web/UI/input dependency, manual-only screenshot proof, committed generated output | Keep acceptance rows aligned with ENG-176 evidence, sample command results, runtime visual foundation floors, and closure diagnostics |
...
| --- | --- | --- | --- |
| Target D3D11 device/swapchain/draw proof | `BlockedByEnv` | L1 RenderScene value-contract rows can stay green while target hardware proof remains L0, but this does not close the L1 runtime visual scene route | Run `windows-hardware-smoke` and `windows-strict-hardware-smoke` on supported production Windows hardware |
...
| L1 runtime visual foundation proof | `StageClose` | L1 cannot be called closed until the shallow-to-deep camera, geometry, scene placement, material/texture, shader/pipeline, animation interpolation, RenderScene, RenderCore/RHI, capture, and missing-layer diagnostic floors pass or report the exact missing YuEngine layer | Implement and run `L1-VIS-001..006` and `L1-SAMPLE-011/012` from `docs/YUENGINE_RUNTIME_VISUAL_FOUNDATION_PLAN.md` and `docs/YUENGINE_L1_VERTICAL_SAMPLE_ACCEPTANCE.md` |
