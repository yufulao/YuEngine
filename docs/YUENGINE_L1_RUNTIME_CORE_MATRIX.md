...
| Render scene | `L1-RSCENE-001..006` | RenderScene owner owns render entity/camera records, full-list visibility, RenderCore packet assembly, multi-entity runtime visual submission, and failure statuses | Object/transform records, Asset handles, L0 RenderCore/RHI, runtime visual material/resource records | RuntimeApp frame context, vertical sample route | FirstSlice | Existing value-contract rows: `RenderScene_BuildsRenderCoreViewPacket`, `RenderScene_FullListVisibilityOutputsOnlyVisibleEntities`, `RenderScene_Missing*`, `RenderScene_NoNativeOrUpperDependency`; missing visual route: `L1-RSCENE-006` runtime multi-entity captured frame proof | `L1-SAMPLE-006` submits render scene through value contracts; `L1-SAMPLE-011/012` must prove or precisely block cube/cylinder/cone runtime visual scene | Target D3D11/display may be `BlockedByEnv`; missing model geometry, material texture slots, shader/pipeline, RenderScene multi-entity, RenderCore multi-draw, camera, capture, or resource resolution is a stage-close runtime blocker | D3D11 includes in public L1 headers, gameplay scene policy, lighting, animation/skinning, UI rendering, editor viewport, standalone D3D sample bypass | Add runtime visual scene proof without editor, Web, UI, or input dependency |
...
| L1 vertical sample prep | `L1-SAMPLE-001..012` | Sample owner aggregates L1 value-contract proof for manifest, runtime boot, object graph, asset/render/audio routes, input, serialize, cleanup, validation, and pure runtime visual scene closure | All L1 subsystem rows above plus L0 explicit hardware statuses and the runtime visual chain from resource/material/model through RenderScene/RenderCore/RHI/camera/capture | RuntimeApp, World, Asset, Input, RenderScene, AudioScene, Serialize, Diagnostics, runtime material/resource records | FirstSlice | Existing value rows: `Sample_L1VerticalPrep_BuildsManifestAndSubmitPrep`, `Sample_L1VerticalPrep_UsesValueContractsWithoutHardware`, `Sample_L1VerticalPrep_DeterministicObjectGraphHasStableSlots`, `Sample_L1VerticalPrep_RoutesAssetRenderAudioAndLifecycle`, `Sample_L1VerticalPrep_ExposesDebugReleaseFastValidationRoute`, `Sample_L1VerticalPrep_RoundTripsStateAndCleansActiveRecords`; missing route: checked-in runtime visual command for `L1-SAMPLE-011/012` | This is the vertical sample linkage row; visual closure requires cube/cylinder/cone full-orbit capture set or exact missing-layer blocker | `docs/YUENGINE_L1_VERTICAL_SAMPLE_ACCEPTANCE.md` exists; real D3D11/XAudio2/XInput remain L0 env blockers, release/native sample runs remain stage-close evidence, and runtime visual scene route is not implemented yet | TouhouNewWorld business logic, old package runtime parsing, UI/GameAdapter/gameplay, editor/Web/UI/input dependency, manual-only screenshot proof, committed generated output | Keep acceptance rows aligned with ENG-176 evidence, sample command results, and runtime visual closure diagnostics |
...
RuntimeApp / FrameContext
    ...
    -> runtime visual scene sample through resource/material/model, camera, and capture values
...
| L1 runtime visual scene proof | `StageClose` | L1 cannot be called closed until the cube/cylinder/cone runtime visual route passes or reports the exact missing YuEngine layer; missing runtime model/material/shader/RenderScene/RenderCore/camera/capture/resource work is not an environment skip | Implement and run the `L1-SAMPLE-011/012` checked-in runtime command from `docs/YUENGINE_L1_VERTICAL_SAMPLE_ACCEPTANCE.md` |
...
7. The pure runtime visual scene route captures the required cube/cylinder/cone
   orbit set, or reports the exact missing runtime layer with no editor, Web,
   UI, or input dependency.
8. Hardware smoke has no unexpected failure; environment skips are graded by
