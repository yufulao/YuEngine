...
| XAudio2 callback | `Src/YuEngine/Audio`, hardware smoke tests, sample audio path | Current hardware smoke passes `Audio_HardwareCallback_CompletesSubmittedBuffer` and `Audio_HardwareCallback_SubmitsPcmStreamQueue` | Done | Supported machine initializes XAudio2, drains PCM queue through callback, reports completion; unavailable device reports explicit status | `ctest --preset windows-hardware-smoke -R "(Audio|XAudio2)" --output-on-failure`; strict preset for target hardware | No output device or XAudio2 unavailable may skip with explicit status on other machines | File IO, sleeps, locks, unbounded allocation in callback, gameplay audio events | Audio hardware owner | Keep callback smoke in final hardware audit and preserve explicit unavailable status |
