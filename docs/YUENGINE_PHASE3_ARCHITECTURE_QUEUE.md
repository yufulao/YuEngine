...
- P3-GATE-017 first slice landed at `5dc9830` with QA PASS and fast gate
  `477/477`.
- P3-GATE-018 is proposed after ENG-071A/B/C converged on a manifest/stream-only
  candidate. It is review-only until explicitly approved and must not authorize
  active restore, object/transform restore, scene load/save policy, package/file
  work, resource loading, or Game Adapter scope.
...
2. Route P3-GATE-018 through review before any implementation task. Its proposed
   scope is manifest/stream-only over caller-owned sidecar records. Do not use
   P3-GATE-017 or P3-GATE-018 to introduce active object/transform restore,
   scene loading, save policy, object construction, resource loading, component
   payload/lifecycle, render/audio/physics/UI/tools/reports, or Game Adapter
   behavior.
