...
`4587c7d1f204663577950241d4c42a5b72ab03a1` records VQ-closed
RuntimeAssetWorldAdapter sidecar failure status after the
...
| Next slice | RuntimeAssetData to RuntimeAssetWorldAdapter handoff | selected by read-only scout `e2b5dd55-a60f-4bbd-b33a-8aa7ef45e75b`, which reports COMPLETE-PASS / `RTSPINE-CANONICAL-DOCS-HEAD-RECONCILE-4587C7D-001-NEXT-GATE-MATRIX-READY`, RuntimeAssetWorldObject(Adapter\|RestoreHandoff) plus WorldSceneAssemblyBridge focused matrix `48/48` PASS, and RuntimeAssetData runtime instance mapping rows `5/5` PASS; next gate is `RTSPINE-RUNTIMEASSETDATA-TO-RUNTIMEASSETWORLDADAPTER-HANDOFF-U64-001`, limited to feeding RuntimeAssetData real `scene_entities`, `scene_transforms`, and `runtime_instance_mappings` into RuntimeAssetWorldAdapter handoff; object handles remain caller-owned World/Object registry state and are not written to asset files; direct WorldObject/editor/GameAdapter/UI/gameplay and broader Resource/File/VFS remain closed |
