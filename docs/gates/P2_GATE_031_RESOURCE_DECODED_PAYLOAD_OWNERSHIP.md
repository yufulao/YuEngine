...
This evidence closes only Resource-owned decoded-payload value/storage behavior
inside the cache/decode chain. It does not complete Resource residency/upload,
RuntimeAsset bridge or CMake cross-proof, RenderScene/RHI, World/editor/importer,
old-package compatibility, real codec/parser behavior, unrelated animation
mapping, Package/Resource public API expansion, adjacent/full Resource rows, or
broad/full CTest.
