...
- RuntimeAsset RTSPINE-008C is a separate Package-focused follow-up outside the
  ...
  does not itself authorize the RuntimeAsset packaged validation bridge; the
  separate 008G gate below does. It does not authorize RTSPINE-008H.
- RuntimeAsset RTSPINE-008D is a separate File/VFS-focused follow-up outside
  ...
  `FileReadRequest` status, output-window, failed-range no-mutation, and async
  ranged output-too-small no-partial-copy behavior only; focused QA task
  ...
  below does. It does not itself authorize the RuntimeAsset packaged validation
  bridge; the separate 008G gate below does. It does not authorize RTSPINE-008H.
- RuntimeAsset RTSPINE-008E is a separate Resource-focused follow-up outside
  ...
  window/reference budget evidence only; focused QA task
  ...
  itself authorize the RuntimeAsset packaged validation bridge; the separate
  008G gate below does. It does not authorize RTSPINE-008H.
- RuntimeAsset RTSPINE-008F is a separate Package-focused follow-up outside
  ...
  diff/hygiene/scope scan PASS, and no broad/full CTest. It does not itself
  authorize the RuntimeAsset packaged validation bridge; the separate 008G gate
  below does. It does not authorize RTSPINE-008H.
- RuntimeAsset RTSPINE-008G is a separate RuntimeAsset-focused follow-up outside
  the P2-GATE-003 first-slice baseline. It is PASS at
  `origin/main@175b6542cf8460b279d1de8a5499e2cbd508c80a` for packaged archive
  byte-range/hash and payload hash preflight before graph-load mutation,
  duplicate load-plan record rejection, packaged validation failure status
  mapping, and ProductRun failure reporting without graph mutation; focused QA
  task `35fdc7a2-c09d-416a-95aa-b4aabdb05d0f` reports focused
  `YuRuntimeAssetDataClosedLoopTests` build PASS, exact 008G rows `5/5` PASS,
  adjacent packaged/product rows `8/8` PASS, committed scope limited to
...
- P2-GATE-029 landed the Audio Resource PCM packet import bridge first slice at
  `0a22dac`. It maps active Resource decode-result metadata with audio
  asset/result classes into caller-owned Audio PCM packet request descriptors,
  records bounded bridge metadata, exposes query/release snapshots, and proves
  deterministic counters without storing decoded bytes or moving Resource
  ownership into Audio core. The landed fast gate is `901/901` PASS with
  `AudioResource` at `8`, `Audio` at `53`, `Resource` at `119`, `Streaming` at
  `36`, `Upload` at `43`, `RHI` at `148`, `RenderCore` at `54`, `Material` at
  `40`, `Fast` at `901`, `PerformanceSmoke` at `108`, `EvidenceOracle` at
  `305`, default `HardwareSmoke` at `0`, and `windows-hardware-smoke` at `7`
  with no AudioResource bridge entries. It does not authorize real codec
  output, decoded byte storage, streaming audio, BGM/SE business IDs, audio
...
- P2-GATE-031 is approved as the next Resource decoded-payload ownership first
  ...
  with no Resource decoded-payload entries. It must not authorize real codec
  output, File/Package/Streaming expansion, RHI upload, Audio lifecycle,
...
- P2-GATE-032 is approved as the next Audio PCM stream queue first slice. It
  ...
  ownership, AudioResource bridge execution, codec decode, decoded byte storage,
  callback submission, streaming audio, BGM/SE business IDs, audio scene,
  UI/World/Script/Game Adapter behavior, native/backend leakage, reports,
  screenshots, logs, sleeps, audible proof, manual proof, hardware-only proof,
  or original-game evidence.
- P2-GATE-033 is approved as an RHI swapchain resize public contract first
  slice. It may add backend-neutral resize request/result values, swapchain
  snapshot resize/reject counters, a capability flag, Null RHI unsupported
  behavior, D3D11 private backbuffer recreation, and color-target generation
  invalidation evidence. The rebased proposal commit is `4021327`; the source
  baseline is `9323358`. It must not authorize Platform event ownership,
  RenderCore resize scheduling, Resource/Streaming/File/Package ownership,
  Audio lifecycle, scene/UI/World/Script/Game Adapter behavior, public native/
  backend leakage, reports, screenshots, logs, sleeps, manual proof,
  hardware-only proof, or original-game evidence.
...
25. Treat the landed P2-GATE-028 Audio PCM sample packet first slice as
    ...
    screenshots, logs, sleeps, audible proof, manual proof, hardware-only proof,
    or original-game evidence.
26. Treat the landed P2-GATE-029 Audio Resource PCM packet import bridge as a
    bridge-owned import boundary. It must stay in cross-module metadata mapping
    from landed Resource decode-result records to landed Audio PCM packet request
    descriptors, and must not authorize Resource ownership inside Audio core,
    Audio packet lifecycle inside Resource core, decoded byte storage, real
    codec output, streaming audio, BGM/SE business IDs, audio scene,
...
30. Treat approved P2-GATE-033 only as an RHI swapchain resize public contract
