...
重要语义修复：`_OP_NEWSLOT/_OP_NEWSLOTA` 是 slot write，不得覆盖 register `a0`，否则 root table 会被污染并丢失 `ModuleBase`。仍有 12 个 engine object calls、21 个 UI helper object calls、8 个 value helper calls、8 个 value method calls、1 个 Module lifecycle hook 需要落成 runtime-owned script/object behavior 和剩余参数 payload 解码，状态是 `trace_ready_not_full_vm`。P4 仍不是完成态，因为 confirmed native、argument/return shape、side effects、oracle/static evidence 和 implementation status 还没有逐行确认；P6 现在已有 C++ service interface baseline 和首批 runtime-owned service state，但还不是完成态，因为完整 API behavior implementation、typed argument/return contracts、`gMenu`/title scene/UI object state 尚未落地；R2 也不是完成态，因为当前只是 title boot-path bytecode/service-state checkpoint，不是完整 Squirrel VM，也没有真实 title UI/gameplay。
...
multi-module title boot-path bytecode/service-state checkpoint，但不是完整 VM。下一条边是继续把
runtime-owned state 扩展到 `gMenu` state、title scene state、UI helper object model、
`MenuObject`/`_menuWindow`/`_listWindow` command buffer，并继续解码剩余 service event 参数 payload，
然后推进 `ModuleTitle.main` 的真实状态分支、scene main dispatch、`MakeNewGame` / `StartGame`。
