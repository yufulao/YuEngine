...
CTest 不允许再回到旧逐项串行全量。旧路径实测 34/34 约 1124 秒、并行 8 约 358 秒；当前默认 CTest 只注册单进程 `yuengine_runtime_contract_suite`，37 个合同约 50 秒。runtime report cache、scene-runtime 复用和 `.sqasm` module cache 已把最深 backend adapter CLI 从 113.06 秒降到 21.46 秒，当前 fast L31 backend-upload-bind 约 25 秒。标准入口：
...
默认编辑循环跑 `tools\verify_runtime.ps1`，当前 fast L31 contract 加 `git diff --check` 约 25 秒。全量 checkpoint 提交前跑 `-Mode full -Jobs 8`；C++ header/ABI/build 改动后跑 `-CleanBuild`。裸 `ctest --test-dir build\cmake-bt143 -C Debug --output-on-failure` 现在是单个 aggregate suite，约 50 秒，可作为 checkpoint 验证。旧逐项 CTest 只在定位单项时显式启用：`cmake -S . -B build\cmake-bt143 -DYUENGINE_ENABLE_LEGACY_CTESTS=ON`。
