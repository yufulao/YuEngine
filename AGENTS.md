...
当前下一步见 `docs/LOOP_TASKS.md`，优先 L8/L9 边界。当前 L7 已有 multi-module title boot/title idle/new-game bytecode/service/script-object state checkpoint；L8 已执行 `mission/sc01/main/ms010_0.b64.sqasm` 的 `setupProcess` 并产出 stage/event/player/camera/checkpoint service state。下一条边是把这些 state 接入资源/stage graph、actor/task、camera task stack、renderer contract，同时继续扩展 Continue/Load/Option/Exit 输入场景。
