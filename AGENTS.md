...
CTest 不允许再默认裸跑串行全量。当前实测串行 34/34 约 1124 秒，并行 8 约 358 秒。标准入口：
...
全量 checkpoint 提交前跑 `-Mode full -Jobs 8`；C++ header/ABI/build 改动后跑 `-CleanBuild`。不要用裸 `ctest --test-dir build\cmake-bt143 -C Debug --output-on-failure` 作为默认循环命令。更深层的性能债是 CLI contract 测试反复重建 evidence chain，后续可做 single-process aggregate runner 或 runtime evidence cache。
