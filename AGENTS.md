...
execution-trace bridge，但不是完整 VM。下一条边是把当前 trace 中的 unresolved/value
缺口转成 runtime contract：实现寄存器/表/对象值语义、UI helper object model、`MenuObject`
命令缓冲和 `GetSaveList` typed behavior，然后推进 `ModuleTitle.main` 的真实状态分支、
