...
L7 当前已有 `script-plan` 和 `script-run`：`script-plan` 解析 `setupProc -> print + modTitle.init`；`script-run --frames 1` 先加载 2 个 baseline modules：`preload.b64` 和 `script/menu/menudef.b64`，再执行 title entry，构造 `modTitle -> ModuleTitle` 和 4 个 canonical 原脚本 scene object，执行 `setupProc`、`ModuleTitle.init` foreach scene init、首帧 `main` boot path。当前 f1 指标：`constructed_objects=3`、`script_methods=58`、`native_obligations=2`、`unique_native_apis=2`、`service_state_events=29`、`save_service_queries=0`、`platform_state_queries=0`、`ui_service_commands=6`、`unresolved_calls=0`。f1 只应触发 `FadeIn`、`PlayBGM` 和 UI object 创建；`GetSaveList/IsFreeDemo` 不属于被动 boot path。

`script-run --frames 2` 已进入真实 title scene idle：`ModuleTitle.main -> modTitle._scenes[0].main -> TitleSceneBase.main -> TitleScene.state0`。关键底层修复：`_scenes` table assignment 规范化到 `modTitle._scenes[index]`；runtime receiver 从 `_OP_PREPCALL/_OP_CALL` 传给后续分发；owner method 对 concrete object 做虚分发，所以 `TitleSceneBase.main` 会进入 `TitleScene.state0`；`_OP_CALL` 实参会传入 recovered methods/constructors；`root.gMenu=table#1` 通过 root table alias 映射回 receiver `gMenu`。当前 f2 passive trace 指标：68 个 state-scanned functions、3,992 条 state-scanned instructions、164 个 object field writes、199 个 table slot writes、151 个 typed call returns、31 个 service state events、0 save/profile queries、0 platform queries、0 unresolved、0 truncated。`script.ScrollWindow@constructor:50._elemCount=5`，`_sel=0`，`gMenu.isDecide(false)`，因此不会再错误落入 `TitleScene.state0 -> PlaySE(3)`。
...
multi-module title boot/title idle bytecode/service/script-object state checkpoint，但不是完整 VM。下一条边是为
`gMenu` 增加 runtime input scenarios，驱动 Continue/New Game/Load/Option/Exit 的原始 bytecode 分支，并继续把
`MenuObject`/`_menuWindow`/`_listWindow` 等 UI helper 调用解码成 command payload，然后推进
`MakeNewGame` / `StartGame` 和 scene/stage load。
