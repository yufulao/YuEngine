...
CTest 不允许再默认裸跑串行全量。当前实测旧串行 34/34 约 1124 秒，并行 8 约 358 秒；runtime report cache、scene-runtime 复用和 `.sqasm` module cache 已把最深 backend adapter CLI 从 113.06 秒降到 21.46 秒。标准入口：
...
默认编辑循环跑 `tools\verify_runtime.ps1`，当前 fast L31 contract 加 `git diff --check` 约 25 秒。全量 checkpoint 提交前跑 `-Mode full -Jobs 8`；C++ header/ABI/build 改动后跑 `-CleanBuild`。不要用裸 `ctest --test-dir build\cmake-bt143 -C Debug --output-on-failure` 作为默认循环命令。更深层的性能债是 full regression 仍启动 35+ 个 CLI 进程，后续可做 single-process aggregate CTest runner。
