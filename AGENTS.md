...
multi-module title boot-path bytecode/service-state checkpoint，但不是完整 VM。下一条边是把 report-only
bytecode state 迁移成 runtime-owned services：`gMenu` state、title scene state、UI helper object model、
`MenuObject`/`_menuWindow`/`_listWindow` command buffer、`GetSaveList` save/profile state、
平台/demo state、音频/淡入 side effects；同时继续解码剩余 service event 参数 payload，然后推进
`ModuleTitle.main` 的真实状态分支、scene main dispatch、`MakeNewGame` / `StartGame`。
