...
当前 trace 已把所有 call 归入 runtime category，`unresolved_calls=0`，`control_flow_unknown=0`，`truncated=false`。Scoped multi-module bytecode state pass 已支持 `_OP_JZ/_OP_JNZ/_OP_JMP/_OP_RETURN` 的 PC-indexed branch execution、`_OP_FOREACH/_OP_POSTFOREACH` 的 title `_scenes` table iteration、method-context object lookup 的 root/global fallback，因此 `ModuleTitle.main` 能通过原 bytecode 解析 `gMenu`，不再靠 pc 硬停。当前指标：38 个 state-scanned functions、3,627 条 state-scanned instructions、31 个 root slot writes、405 个 class slot writes、121 个 object field writes、198 个 table slot writes、144 个 typed call returns、0 个 UI object mutations。当前还输出 57 个 service state events，并同步写入 runtime-owned service state：4 个 save/profile queries、2 个 platform state queries、1 个 audio command、1 个 scene fade command、20 个 tracked UI objects、24 个 UI service commands、4 个 save/value state queries、2 个 decoded service argument payloads；状态快照包含 `save.empty_save_list_queries=4`、`save.save_list_count_queries=4`、`save.save_list_entries=0`、`platform.flags.IsFreeDemo=false`、`audio.current_bgm_id=3`、`scene.fade_in_duration=0.7`、`scene.fade_in_blend=0`、`ui.created_objects=20`、`ui.command_count=24`。`FadeIn` 已通过 `ModuleBase._fadeInTime` 解码为 `duration=0.7; blend=0`。`script-run` 还输出 runtime-owned script state：31 个 root fields、28 个 runtime objects、17 个 runtime tables、24 个 class slot tables、16 条 class base links；`root.gMenu=table#1` 且该表有 87 个字段，`root.modTitle=object:modTitle`，`modTitle._nextState=300`，`modTitle._scenes[0..3]` 保持 concrete classes：`TitleScene/NewGameScene/LoadScene/OverwriteSaveScene`。

重要语义修复：`_OP_NEWSLOT/_OP_NEWSLOTA` 是 slot write，不得覆盖 register `a0`，否则 root table 会被污染并丢失 `ModuleBase`。root slot 写入现在会把 `modTitle = ModuleTitle()` 规范化到 canonical `modTitle` 对象，基类/super 方法执行也不会覆盖对象的 concrete class。仍有 12 个 engine object calls、21 个 UI helper object calls、8 个 value helper calls、8 个 value method calls、1 个 Module lifecycle hook 需要落成 runtime-owned script/object behavior 和剩余参数 payload 解码，状态是 `trace_ready_not_full_vm`。P4 仍不是完成态，因为 confirmed native、argument/return shape、side effects、oracle/static evidence 和 implementation status 还没有逐行确认；P6 现在已有 C++ service interface baseline、首批 runtime-owned service state、首批 runtime-owned script/object state，但还不是完成态，因为完整 API behavior implementation、typed argument/return contracts、`gMenu` 行为消费和 UI command payload 尚未落地；R2 也不是完成态，因为当前只是 title boot-path bytecode/service-state checkpoint，不是完整 Squirrel VM，也没有真实 title UI/gameplay。
...
`gMenu` table state、canonical `modTitle` 和 title scene objects 来推进 `ModuleTitle.main` 的真实状态分支，
