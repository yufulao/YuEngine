...
当前已达到 R1 诊断启动，并完成 L2 资源依赖诊断、L3 脚本模块模型、L4 native service interface baseline、L5 service-backed runtime lifecycle、L6 oracle capture readiness，并启动 L7 title script execution，不是游戏执行：C++ runtime spine 已能通过 `project.json` 加载原 sample 和 empty sample，挂载 VFS，解析 pack manifest 索引，加载 startup `.sqasm` 诊断，并通过 runtime-owned 11 个 native service interfaces 报告 native/API obligations。已验证原 sample：7 个 boot phases、2 个 loose mounts、13,028 个 pack resources、84 个 native registry APIs、11 个 native services、39 个 startup obligations，全部仍是 `not_started`。L2 已验证 title background/logo/DLC stems、title script resource refs、first-mission stage/rail-camera resources 都能通过 VFS 解析，resource report required missing 为 0。L3 已验证 title module 为 81 functions / 4,466 instructions / 593 calls / 80 closure bindings，first mission candidate 为 62 functions / 4,068 instructions / 640 calls / 61 closure bindings，且 C++ parser 可在秒级输出结构化报告。L4 已验证 11 services / 84 native APIs / 0 unowned APIs / 0 unbound APIs，但 native 行为仍没有实现。L5 已验证 original 和 empty sample 都是 `ok=true phases=7 failed_phases=0`。L6 已验证原 exe 可定位，但无 file-IO/render 捕获工具且没有用户驱动 title boot 记录，所以 P1 仍不是完成态；不要伪造 oracle。L7 当前已有 `script-plan` 和 `script-run`：`script-plan` 解析 `setupProc -> print + modTitle.init`；`script-run --frames 1` 先加载 2 个 baseline modules：`preload.b64` 和 `script/menu/menudef.b64`，再执行 title entry，构造 `modTitle -> ModuleTitle` 和 4 个原脚本 scene object，执行 `setupProc`、`ModuleTitle.init` foreach scene init、首帧 `main` boot edge，并通过 `NativeServiceCatalog` 触发 12 个 native/API obligations，覆盖 `FadeIn`、`PlayBGM`、`MenuObject`、`GetSaveList`、`IsFreeDemo`、`IsOverDemo`；当前 trace 已把所有 call 归入 runtime category，`unresolved_calls=0`，并且执行 scoped multi-module bytecode state pass：38 个 state-scanned functions、4,087 条 state-scanned instructions、31 个 root slot writes、405 个 class slot writes、132 个 object field writes、213 个 table slot writes、150 个 typed call returns、15 个 UI object mutations。当前还输出 67 个 service state events：4 个 save/profile queries、4 个 platform state queries、1 个 audio command、1 个 scene fade command、21 个 tracked UI objects、24 个 UI service commands、12 个 save/value state queries、6 个 decoded service argument payloads，且 `FadeIn` 已通过 `ModuleBase._fadeInTime` 解码为 `duration=0.7; blend=0`。重要语义修复：`_OP_NEWSLOT/_OP_NEWSLOTA` 是 slot write，不得覆盖 register `a0`，否则 root table 会被污染并丢失 `ModuleBase`。仍有 12 个 engine object calls、21 个 UI helper object calls、8 个 value helper calls、16 个 value method calls、1 个 Module lifecycle hook 需要落成 runtime-owned service 行为和剩余参数 payload 解码，状态是 `trace_ready_not_full_vm`。P4 仍不是完成态，因为 confirmed native、argument/return shape、side effects、oracle/static evidence 和 implementation status 还没有逐行确认；P6 现在已有 C++ service interface baseline，但还不是完成态，因为 API behavior implementation、typed argument/return contracts 和 service state 尚未落地；R2 也不是完成态，因为当前只是 title boot-edge bytecode/service-state checkpoint，不是完整 Squirrel VM，也没有真实 title UI/gameplay。
...
multi-module title boot-edge bytecode/service-state checkpoint，但不是完整 VM。下一条边是把剩余 service state
event 的参数 payload 解码出来，并迁移成 runtime-owned services：UI helper object model、
`MenuObject`/`_menuWindow`/`_listWindow` command buffer、`GetSaveList` save/profile state、
平台/demo state、音频/淡入 side effects，然后推进 `ModuleTitle.main` 的真实状态分支、
