...
title boot-edge bytecode-state checkpoint，但不是完整 VM。下一条边是把当前 bytecode state
和 runtime category 转成具体 service behavior：实现 UI helper object model、`MenuObject`
命令缓冲、`GetSaveList` typed behavior、平台/demo state、音频/淡入 side effects，然后推进
